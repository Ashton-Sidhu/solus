...
describe.serial('session emitter', () => {
  test('records a Claude turn with natural-duration child spans', () => {
    ...
    expect(spans.map((span) => span.kind)).toEqual([
      'setup', 'turn', 'response_stream', 'tool_call', 'tool_input', 'tool_execution', 'turn_settlement',
    ])
    ...
    expect(attrs(tool)).toMatchObject({ input: '{"command":"bun test"}' })
    expect(tool).toMatchObject({ started_at: 1_030, ended_at: 1_042 })
    expect(spans.find((span) => span.kind === 'tool_input')).toMatchObject({
      parent_span_id: tool.span_id, started_at: 1_030, ended_at: 1_040,
    })
    expect(spans.find((span) => span.kind === 'tool_execution')).toMatchObject({
      parent_span_id: tool.span_id, started_at: 1_040, ended_at: 1_042,
    })
  ...
  test('groups a permission wait and tool phases under the tool call', () => {
    // WHY: Claude streams large tool inputs before it calls the MCP server.
    // The parent keeps end-to-end latency while its children show where that
    // time went without flattening permission waits into the turn.
    const emitter = new emitterModule.SessionEmitter()
    emitter.beginTurn({ sessionId: 'tool-phases', prompt: 'comment', promptSource: 'typed', startedAt: 6_000 })
    emitter.completeSetup('tool-phases', {
      provider: 'claude-code', model: 'claude', projectRoot: '/repo', origin: 'typed', isResume: false,
    }, 6_005)
    emitter.onEvent('tool-phases', {
      type: 'tool_call', toolName: 'mcp__solus__comment_task', toolId: 'comment-1', index: 0,
    }, 6_010)
    emitter.onEvent('tool-phases', {
      type: 'tool_call_complete', toolId: 'comment-1', index: 0, toolInput: '{"body":"status"}',
    }, 6_020)
    emitter.onEvent('tool-phases', {
      type: 'permission_request', questionId: 'permission-comment', toolName: 'mcp__solus__comment_task',
      toolUseId: 'comment-1', options: [{ id: 'allow', label: 'Allow', kind: 'allow' }],
    }, 6_021)
    emitter.resolvePermission('tool-phases', 'permission-comment', 'allow', 6_030)
    emitter.onEvent('tool-phases', {
      type: 'tool_result', toolUseId: 'comment-1', content: 'Comment added.',
    }, 6_040)
    emitter.recordTerminal('tool-phases', 'ok', 6_045)
    emitter.finishTurn('tool-phases', 'completed', 6_045)

    const recorded = rows()
    const tool = recorded.find((span) => span.kind === 'tool_call')!
    const input = recorded.find((span) => span.kind === 'tool_input')!
    const permission = recorded.find((span) => span.kind === 'permission_wait')!
    const execution = recorded.find((span) => span.kind === 'tool_execution')!
    expect(tool).toMatchObject({ started_at: 6_010, ended_at: 6_040 })
    expect(input).toMatchObject({ parent_span_id: tool.span_id, started_at: 6_010, ended_at: 6_020 })
    expect(permission).toMatchObject({ parent_span_id: tool.span_id, started_at: 6_021, ended_at: 6_030 })
    expect(execution).toMatchObject({ parent_span_id: tool.span_id, started_at: 6_030, ended_at: 6_040 })
  })
