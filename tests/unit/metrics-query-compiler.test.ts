...
function run(spec: MetricsQuerySpec) {
  ...
  return sqlGuard.runCompiledSql(compiled.sql, compiled.params, compiled.sourceView)
}

/** Most of these tests are about what the compiler names its output columns. */
function columnNames(result: { columns: Array<{ name: string }> }): string[] {
...
describe.serial('metrics query compiler', () => {
  test('counts and averages grouped by a promoted column', () => {
    ...
    expect(columnNames(result)).toEqual(['model', 'count', 'avg_duration_ms'])
  ...
  test('buckets by day', () => {
    ...
    expect(columnNames(result)).toEqual(['bucket', 'sum_duration_ms'])
    expect(result.rows).toEqual([
      ['1970-01-01', 9_000],
      ['1970-01-02', 7_000],
    ])
  ...
  test('computes nearest-rank percentiles beside plain aggregates', () => {
    ...
    expect(columnNames(result)).toEqual(['name', 'count', 'p50_duration_ms', 'p95_duration_ms', 'max_duration_ms'])
  ...
  test('percentile with no group keys returns a single row', () => {
    ...
    expect(columnNames(result)).toEqual(['p95_ms'])
  ...
  test('lists spans without aggregates, newest first, capped by limit', () => {
    ...
    expect(columnNames(result)).toContain('attrs')
    const spanIds = result.rows.map((row) => row[columnNames(result).indexOf('span_id')])
...
// The registry declares each view's column types and generates the SQL views
// from that declaration. A result carries those types through to the client so
// nothing downstream has to infer a column's meaning from its cells. A column
// the query invented has no declaration and is reported untyped, which is the
// signal to the client that it must read the values itself.
describe.serial('declared column types', () => {
  function columnType(result: { columns: Array<{ name: string; type?: string }> }, name: string) {
    return result.columns.find((column) => column.name === name)?.type
  }

  test("a listing carries its view's declared types", () => {
    const result = run({ filters: [{ field: 'kind', op: 'eq', value: 'turn' }], limit: 1 })

    expect(columnType(result, 'started_at')).toBe('number')
    expect(columnType(result, 'duration_ms')).toBe('duration')
    expect(columnType(result, 'status')).toBe('string')
  })

  test('an aliased aggregate is untyped — no registry field declares it', () => {
    const result = run({
      filters: [{ field: 'kind', op: 'eq', value: 'turn' }],
      groupBy: [{ field: 'model' }],
    ...
    // The grouped dimension is a real column and keeps its type; the average is
    // the query's own invention and carries none.
    expect(columnType(result, 'model')).toBe('string')
    expect(columnType(result, 'avg_duration_ms')).toBeUndefined()
  })

  test('a spec that declares no grain types nothing', () => {
    const result = run({ aggregates: [{ fn: 'count' }] })

    expect(result.columns.every((column) => column.type === undefined)).toBe(true)
  })
})
