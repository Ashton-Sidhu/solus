...
describe("toast service", () => {
  ...
  test("stacks toasts instead of replacing the one before", () => {
    let committed = 0
    const first = toasts.undo("Deleted", () => {}, { onDismiss: () => committed++ })
    const second = toasts.info("Connected")

    // A new toast must not settle a live one: the delete's deferred commit is
    // still waiting on that toast's own undo window, not on the next toast.
    expect(committed).toBe(0)
    expect(dismissed).toEqual([])
    expect(toasts.isActive(first)).toBe(true)
    expect(toasts.isActive(second)).toBe(true)
  })

  test("a bare dismiss clears every toast on screen", () => {
    const first = toasts.info("Connecting…")
    const second = toasts.info("Connected")

    expect(toasts.dismiss()).toBe(true)
    expect(dismissed).toEqual([first, second])
    expect(toasts.dismiss()).toBe(false)
  })

  test("reusing an ID replaces that toast in place and commits it", () => {
    let committed = 0
    toasts.show({ message: "Saving…", id: "slot", onDismiss: () => committed++ })
    toasts.show({ message: "Saved", id: "slot" })

    expect(committed).toBe(1)
    // Sonner swaps the slot's content, so nothing is dismissed out from under it.
    expect(dismissed).toEqual([])
    expect(toasts.isActive("slot")).toBe(true)
  })
