...
describe.serial('Claude turn capture end to end', () => {
  test('commits main-thread prose once before the next tool event', async () => {
    ...
    const lifecycle = await plane.runTurn({
      target: { kind: 'new-session' }, sessionId: 'solus-complete-prose', input: input(null), tools: [],
    ...
    backend.send('claude-session-1', { type: 'text_chunk', text: 'Complete ' })
    backend.send('claude-session-1', { type: 'text_chunk', text: 'prose.' })
    expect(delivered.map((event) => event.type)).toEqual(['text_pending'])

    backend.send('claude-session-1', {
      type: 'tool_call', toolName: 'Read', toolId: 'toolu_boundary', index: 0,
    })
    expect(delivered).toMatchObject([
      { type: 'text_pending' },
      { type: 'text_chunk', text: 'Complete prose.' },
      { type: 'tool_call', toolName: 'Read', toolId: 'toolu_boundary' },
    ])

    backend.settle('claude-session-1')
    await lifecycle.done

    const recorded = spans('solus-complete-prose')
    const turnAttrs = JSON.parse(recorded.find((row) => row.kind === 'turn')!.attrs)
    expect(turnAttrs).toMatchObject({
      response: 'Complete prose.',
      responseChars: 15,
      timeToFirstTextMs: expect.any(Number),
      timeToFirstActivityMs: expect.any(Number),
    })
    expect(recorded.filter((row) => row.kind === 'response_stream')).toHaveLength(1)
    plane.shutdown()
  })
  ...
  test('a full Claude turn writes natural-duration child spans to metrics.db', async () => {
    ...
    expect(rows
      ...
      'response_stream', 'setup', 'thinking', 'tool_call', 'tool_execution', 'tool_input', 'turn', 'turn_settlement',
    ])
    ...
    expect(rows.filter((row) => row.kind === 'tool_input' || row.kind === 'tool_execution')
      .every((row) => row.parent_span_id === tool.span_id)).toBe(true)
