...
describe('normalizeCodexNotification', () => {
  ...
  test('holds a long-running command open until the process exits', () => {
    // WHY: this is how a slow Codex command stays visible. `command/exec` defers
    // its response until the process exits, so the tool call carries the whole
    // run and the activity row keeps naming it. Claude's backgrounded Bash
    // answers at launch instead and needs `describeBackgroundWait` to say the
    // same thing; if Codex ever started settling early it would inherit that
    // blind spot silently.
    const normalizer = new CodexTurnNormalizer({ planMode: false })
    const poll = 'for i in $(seq 1 30); do dig +short www.solus.sh; done'

    expect(normalizer.push({
      method: 'item/started',
      params: {
        startedAtMs: 500_000,
        item: { id: 'cmd-1', type: 'commandExecution', command: poll, status: 'inProgress' },
      },
    })).toEqual([expect.objectContaining({
      type: 'tool_call', toolId: 'cmd-1', toolInput: poll, startedAtMs: 500_000,
    })])

    // Output arriving mid-run must not settle the call.
    expect(normalizer.push({
      method: 'item/updated',
      params: { itemId: 'cmd-1', delta: 'attempt 1: https=000\n' },
    }).some((event) => event.type === 'tool_call_complete')).toBe(false)

    expect(normalizer.push({
      method: 'item/completed',
      params: {
        completedAtMs: 775_000,
        item: { id: 'cmd-1', type: 'commandExecution', command: poll, status: 'completed' },
      },
    }).some((event) => event.type === 'tool_call_complete')).toBe(true)
  })
})
