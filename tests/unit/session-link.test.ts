...
describe("session links", () => {
  test("uses indexed metadata so a linked session restores its real project and branch directory", async () => {
    const indexed: SessionMeta = {
      ...
      size: 42,
      cwd: "/repo/.git/solus/worktrees/feature",
      projectPath: "-repo--git-solus-worktrees-feature",
      isWorktree: true,
      projectRoot: "/repo",
    };
  ...
  test("a reference typed in the input bar is a session link, not a dead url", () => {
    // WHY: the picker serializes every reference as `<scheme>://ref`, while the
    // agent's session tools write `session://open`. Both must reach the chip —
    // an unparsed href renders as a plain anchor that opens nothing.
    const typed = parseSessionHref(
      "session://ref?sessionId=linked-session&provider=codex&cwd=%2Frepo&serverId=laptop",
    );
    ...
    // Any other host is still not a session destination.
    expect(
      parseSessionHref("session://close?sessionId=a&provider=codex"),
    ).toBeNull();
  });
