...
// The two renderer source trees. `src/renderer` became
// `packages/workspace-ui/src` in the packages split (ADR-0014); the roots were
// not moved with it, so every one of these tests spent that time throwing
// ENOENT instead of failing — which is how a surface-worth of `text-[11.5px]`
// got in without CI noticing. `repoPath` below already spelled the new
// location, so only the read side was stale.
const roots = [
  join(import.meta.dir, '../../packages/workspace-ui/src'),
...
describe('renderer type scale', () => {
  ...
  /**
   * Files carrying off-scale sizes that predate the roots being repointed.
   *
   * This list should not exist, and it exists for one reason: the roots above
   * pointed at `src/renderer` after the packages split, so every test in this
   * file threw ENOENT rather than failing. The debt below accumulated behind
   * that, unseen. Recording it is the honest alternative to either deleting the
   * rule or silently migrating twelve surfaces nobody asked about.
   *
   * It is keyed by *file*, like the two ratchets under it, and `both new
   * ratchets only tighten` fails if a listed file is clean — so a surface
   * cannot be fixed and then quietly regrow. It may only shrink. Crucially it
   * does not blunt the rule where it earns its keep: a file outside this list
   * still fails on the first off-scale size, which is every file anyone writes
   * from here on.
   */
  const offScalePending: string[] = JSON.parse(
    readFileSync(join(import.meta.dir, 'fixtures/off-scale-pending.json'), 'utf8'),
  )
  const OFF_SCALE_PENDING = new Set(offScalePending)

  test('nothing anywhere renders at a size off the scale', () => {
    // WHY: a value the design does not contain is a rendering defect, not
    // untidiness — so it is deliberately *not* covered by the redundancy
    // ratchet. Exempting a defect because the file is pending for a different
    // rule is how PrsPage kept nine 13px buttons through the migration that
    // retired 13px; `offScalePending` is a separate list for exactly that
    // reason, and being on it buys a file nothing under any other rule.
    const offenders: string[] = []
    for (const { path, source } of allSources()) {
      if (OFF_SCALE_PENDING.has(repoPath(path))) continue
      offenders.push(...defects(path, source))
    }
  ...
  test('both new ratchets only tighten, and name files that exist', () => {
    const known = new Set([...allSources()].map(({ path }) => repoPath(path)))
    const stale = [...chromePending, ...rungOverridePending, ...offScalePending].filter(
      (e) => !known.has(e),
    )
    ...
    for (const { path, source } of allSources()) {
      ...
      if (OFF_SCALE_PENDING.has(repo) && defects(path, source).length === 0) migrated.push(repo)
    }
