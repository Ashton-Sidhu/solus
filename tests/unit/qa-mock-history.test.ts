...
for (const provider of ['claude-code', 'codex'] as const) {
  describe(provider, () => {
    test('distinct sessions keep history isolated and resume after host restart', () => {
      ...
      expect(restored.begin({ conversation: { kind: 'resume', threadId: first }, prompt: 'follow-up', cwd: '/fixture' })).toBe(first)
    ...
    test('fork copies history without modifying its source', () => {
      ...
      const fork = history.begin({ conversation: { kind: 'fork', sourceThreadId: source }, prompt: 'fork', cwd: '/fixture' })
