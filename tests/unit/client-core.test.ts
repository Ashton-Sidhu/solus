...
describe('client core transport helpers', () => {
  ...
  test('a bare address means the Solus port, not a web server on port 80', () => {
    // Typing the LAN address of a host is the most common way to pair a phone,
    // and port 80 is never where Solus answers.
    expect(normalizeServerUrl('10.10.1.219')).toBe('http://10.10.1.219:3000')
    expect(normalizeServerUrl('my-mac.local/')).toBe('http://my-mac.local:3000')
    // An explicit scheme is the user being specific; a proxy on 443 is real.
    expect(normalizeServerUrl('https://solus.example.com')).toBe('https://solus.example.com')
    expect(normalizeServerUrl('http://10.10.1.219')).toBe('http://10.10.1.219')
  })
