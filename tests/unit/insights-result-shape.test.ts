...
// Columns are named but untyped, which is what a free-form SQL result looks
// like: the classification these tests exercise is the fallback that reads the
// cells. `declared` below covers the typed path.
function result(
  names: string[],
...
describe('declared grain vs the legacy column sniff', () => {
  test('a declared turns grain with the turn columns is the turn listing', () => {
    ...
    expect(resultRendering(shaped, SCHEMA)).toEqual({ rendering: 'turns' })
  })
  ...
  test('an event listing that selects trace_id is NOT read as turns — the misdetection the declared grain exists to fix', () => {
    ...
    expect(resultRendering(shaped, SCHEMA)).toEqual({ rendering: 'events', view: 'events', kind: 'tool_call' })
  })
  ...
  test('without a declared grain (an older host), the turn-column sniff still applies', () => {
    ...
    expect(resultRendering(legacy, SCHEMA)).toEqual({ rendering: 'turns' })
  })
  ...
  test('a declared non-turn grain without started_at falls through to the grid', () => {
    ...
    expect(resultRendering(rollup, SCHEMA).rendering).toBe('table')
  })
  ...
  test('an events grain survives a missing schema — only the kind colour is lost', () => {
    ...
    expect(resultRendering(shaped, null)).toEqual({ rendering: 'events', view: 'events', kind: '' })
  })
})
...
describe('the event kind the multi-kind view derives', () => {
  ...
  test('a mixed-kind listing stays generic rather than wearing one kind\'s colour', () => {
    ...
    expect(resultRendering(shaped, SCHEMA)).toEqual({ rendering: 'events', view: 'events', kind: '' })
  })
  ...
  test('without a kind column, a single-kind view still names its kind', () => {
    ...
    expect(resultRendering(shaped, SCHEMA)).toEqual({
      rendering: 'events',
      view: 'internal_events',
      kind: 'internal.rpc',
    })
