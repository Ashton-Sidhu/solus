...
describe('range conditions in generated SQL', () => {
  ...
  test('a compiled range predicate can move without another NL compile', () => {
    // WHY: NLP authors the SQL once. A time-filter change owns only the exact
    // predicate it asked for and must not spend another model call.
    const formatted = `select model, count(*)\nfrom turns t\nwhere t.started_at > (strftime('%s', 'now') * 1000 - ${DAY})\ngroup by model`
    const rewritten = replaceRangeCondition(formatted, RELATIVE, ABSOLUTE)

    expect(rewritten).toContain(
      `t.started_at >= ${ABSOLUTE.from} and t.started_at < ${ABSOLUTE.to}`,
    )
    expect(replaceRangeCondition(rewritten ?? '', ABSOLUTE, RELATIVE)).toContain(
      `t.started_at > (strftime('%s','now') * 1000 - ${DAY})`,
    )
    expect(replaceRangeCondition('select count(*) from turns', RELATIVE, ABSOLUTE)).toBeNull()
  })
