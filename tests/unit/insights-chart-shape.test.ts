...
import { resultRendering } from '@solus/workspace-ui/components/insights/lib/result-shape'
 ...
 *  `resultRendering` takes one to decide the grain first. */
...
describe('trend detection', () => {
  test('a bucketed aggregate becomes a line over parsed bucket labels', () => {
    ...
    const rendering = resultRendering(shaped, SCHEMA)
    expect(rendering.rendering).toBe('trend')
    if (rendering.rendering !== 'trend') return
    expect(rendering.trend.mark).toBe('line')
    expect(rendering.trend.valueFormat).toBe('duration')
    expect(rendering.trend.seriesColumn).toBeNull()
    expect(rendering.trend.lines).toHaveLength(1)
    // Sorted by time regardless of the query's own order.
    expect(rendering.trend.lines[0].points.map((point) => point.value)).toEqual([700, 800, 900])
  })
  ...
  test('a single point is not a trend', () => {
    ...
    expect(resultRendering(shaped, SCHEMA).rendering).toBe('table')
  })
...
describe('the breakdown dimension a grouped result carries', () => {
  ...
  test('a grouped measure draws one line per dimension value, not one through all rows', () => {
    ...
    expect(rendering.rendering).toBe('trend')
    if (rendering.rendering !== 'trend') return
    expect(rendering.trend.seriesColumn).toBe('model')
    expect(rendering.trend.valueColumn).toBe('token_spend_usd')
    expect(rendering.trend.lines.map((line) => line.label)).toEqual([
      'claude-fable-5',
      'claude-opus-5',
    ])
    expect(rendering.trend.lines[1].points.map((point) => point.value)).toEqual([15.0, 9.9])
    // Each point names its own line, so the tooltip can say which it is.
    expect(rendering.trend.lines[0].points[0].series).toBe('claude-fable-5')
  })
...
describe('which measure the chart draws', () => {
  ...
  test('the choice threads through the shape the page renders', () => {
    const rendering = resultRendering(shaped, SCHEMA, 'calls')
    expect(rendering.rendering).toBe('trend')
    if (rendering.rendering !== 'trend') return
    expect(rendering.trend.valueColumn).toBe('calls')
  })
})
...
describe('a rollup with no time column is a ranking', () => {
  ...
  test('a time column keeps it a trend; the page asks for the trend first', () => {
    ...
    expect(resultRendering(slowestTools, SCHEMA).rendering).toBe('ranking')
  })
