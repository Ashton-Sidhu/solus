...
describe('turn collapse', () => {
  ...
  test('the live row only appears when nothing else is reporting the run', () => {
    ...
    // Completed prose does not carry live chrome, so the row reports that the
    // still-running session is writing its next prose run.
    expect(
  ...
  test('keeps a direct review guide visible after its request row', () => {
    ...
    expect(turn.visibleWhenCollapsed.map((item) => item.kind)).toEqual(['review-guide'])
    expect(turn.tail.map((item) => item.kind)).toEqual(['assistant'])
  })
...
describe('a run parked on a card has not ended', () => {
  test('keeps the turn live while a question, permission or plan is unanswered', () => {
    ...
    expect(runIsLive('awaiting_input')).toBe(true)
    expect(runIsLive('awaiting_plan')).toBe(true)
    expect(runIsLive('running')).toBe(true)
  })

  test('a settled run folds', () => {
...
describe('document stack', () => {
  ...
  test('a rendered artifact keeps its frame even though it now names a work', () => {
    // WHY: an artifact is persisted as a work and carries a workRef so its
    // frame can open it in a pane. The frame is the point of an artifact —
    // folding it into a document stack would hide the render behind a title.
    const items = groupMessages([
      msg({ role: 'user', content: 'chart the latency' }),
      tool('render_artifact', '{"title":"Latency"}'),
      msg({
        role: 'assistant',
        artifact: { kind: 'html', html: '<html></html>' },
        workRef: { workId: 'w-art', title: 'Latency', workType: 'artifact' },
      }),
      work('w1', 'Notes'),
    ])

    expect(items.map((item) => item.kind)).toEqual(['user', 'tool-group', 'artifact', 'document'])
  })

  test('works written back to back are one stack, and the tool rows between them do not break it', () => {
    // WHY: `create_work` is a tool call followed by its work message, so the raw
    // transcript always interleaves tool rows. If those closed the stack, a
    // three-file write would render as three cards — the thing the stack exists
    // to replace.
    const items = groupMessages([
      msg({ role: 'user', content: 'write up the drift' }),
      tool('create_work', '{"title":"typography tokens"}'),
      work('w1', 'Typography tokens'),
      tool('create_work', '{"title":"migration plan"}'),
      work('w2', 'Migration plan'),
      tool('create_work', '{"title":"rollout checklist"}'),
      work('w3', 'Rollout checklist'),
    ])

    const documents = items.filter((item) => item.kind === 'document')
    expect(documents).toHaveLength(1)
    expect(documents[0].kind === 'document' && documents[0].messages.map((m) => m.workRef?.workId)).toEqual([
      'w1',
      'w2',
      'w3',
    ])
    // The stack sits where the FIRST write happened and grows in place — the
    // later tool rows land after it instead of pushing a second card.
  ...
  test('a stack stays visible when its turn folds, and each stack keys apart', () => {
    const [turn] = turnsFor([
      ...
      work('w1', 'First'),
      work('w2', 'Second'),
      msg({ role: 'assistant', content: 'Both are written.' }),
    ])
