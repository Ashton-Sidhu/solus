...
- **Design priority:** follow the user's prompt first, then the target product or project design system, then Solus chrome integration. When the artifact represents Solus-native data or a general assistant visualization, use Solus theme variables and the local palette below. When the artifact compares UI directions, mocks another product, or reviews a branded surface, match the target context enough for a fair evaluation while keeping the outer body transparent and respecting light/dark mode.
- **Mocking the user's own UI:** when the artifact mocks up, redesigns, or compares the user's product UI, read that project's real tokens and components first (theme files, CSS variables, Tailwind config, the component source) and reproduce them accurately, including their light and dark values and their edge and shadow treatment. Do not substitute the `--solus-art-*` palette for the product's own fills, and do not flatten a surface the product draws with a shadow. A "current" panel that does not look like the real app makes the comparison unfair.
...
- **Theme:** Solus injects its OWN palette into the frame as CSS variables and sets the matching color-scheme for the active light/dark theme — so just use them and the artifact matches the app in both modes. Text: `--solus-text-primary` / `--solus-text-secondary` / `--solus-text-tertiary`. Accent: `--solus-accent` plus `--solus-accent-soft` / `--solus-accent-light` / `--solus-accent-border`. Neutrals, for artifacts with no design of their own (charts, diagrams, explainers, data views): `--solus-art-surface` and `--solus-art-raised` (soft ivory panels), `--solus-art-border` and `--solus-art-border-strong` (hairlines). Use fills sparingly — most structure reads better as a hairline or whitespace than as a filled panel. Keep the body background transparent and never hardcode a hex that would vanish on the opposite theme.
- **Color:** do not hardcode raw greys (no #888 / #ccc / gray-500 / rgba(0,0,0,…)) — they drift from the app's neutrals and break on the opposite theme. Structural and "neutral" elements use the neutrals above (`--solus-art-border`, `--solus-text-tertiary`). For categorical / multi-series data use the brand data palette IN ORDER: `--solus-art-1` terracotta, `--solus-art-2` amber, `--solus-art-3` sage, `--solus-art-4` teal, `--solus-art-5` dusty blue, `--solus-art-6` plum (all tuned to the warm theme). For good/bad meaning use `--solus-art-positive` (green) and `--solus-art-negative`. Encode meaning, do not cycle a rainbow: same category → same colour; a single-series chart should be one colour (the accent), not many.
