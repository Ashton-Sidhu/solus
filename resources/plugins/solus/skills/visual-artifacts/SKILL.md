...
5. As your LAST step, call the `render_artifact` tool with the finished HTML and a short `title` to render it. The artifact is saved as a work under that title: it appears in the works gallery, links to the session's task, and `update_work` revises it by the `work_id` the tool returns — never render a second copy to change one.
...
When the HTML is ready, call `render_artifact` with it and a `title` as the final step, after you have explained what you built. Also set the document `<title>` to the same text; it is the fallback name when `title` is omitted.
