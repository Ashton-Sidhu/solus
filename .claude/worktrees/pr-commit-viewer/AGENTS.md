...
- `review.ts`, `task-types.ts`, `run-types.ts`, `git-types.ts`, `providers.ts`,
...
- **`contexts/`:** state stores and contexts.
  ...
  - `run.store`, `tasks.store`, `works.store`, `automations.store`, `prs.store`,
...
| `project-panel/` | Git, Run, Tasks, Works, and Automations panels plus log console |
...
| `pickers/`, `command-palette/`, `files/`, `comments/`, `connections/`, `run/` | Focused interaction surfaces |
