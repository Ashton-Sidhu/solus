...
describe('paged history expansion', () => {
  test('automatic backfill widens the window instead of reading the whole session', async () => {
    ...
    await store.expandHistory('tab-1', { paged: true })
    await store.expandHistory('tab-1', { paged: true })

    expect(requestedLimits).toEqual([400, 600])
    expect(session.historyTruncated).toBe(true)

    // A deliberate user gesture still reads everything that is left.
    await store.expandHistory('tab-1')
