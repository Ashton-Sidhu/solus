<script lang="ts">
  ...
  const rowHoverWash =
    "hover:bg-[color-mix(in_srgb,var(--solus-accent)_6%,transparent)] hover:border-[color-mix(in_srgb,var(--solus-accent)_8%,transparent)]";
...
>
  ...
  >
    {#snippet rowActions(tabIds: string[], label: string, showNew: boolean)}
      <span
    ...
    <Sidebar.Menu class="gap-1">
      {#each sidebarStore.projectBranchGroups as pg (pg.projectKey)}
        ...
        >
          <span
            class="text-(--solus-text-tertiary) transition-transform duration-150 flex-shrink-0 flex items-center {!collapsed
