...
Presence is what makes a shared host feel shared: who is here, where they are, whether they are typing, and whose turn is running. This slice adds it for sessions and for the host as a whole. Documents and diagrams get cursors and selections later, with Yjs; nothing here depends on that.
...
- Surfaces: the session band before Share; the transcript tail beside the queued prompts; user bubbles, including the held prompts in the queue; task session rows in the sidebar (from the host roster, so a row can say who is in a session this client has not opened); the task page's Sessions section, table and phone cards alike (faces, the draft dot, and the ring on the turn's author, from the host roster's `activity`); the work header before Publish; the sidebar navigation's last row; the phone drawer between the pins and the tasks.
...
Document carets and selections, diagram pointers (both wait for Yjs awareness), a reading marker in the minimap, and any presence for guests beyond their one session's room. Follow mode follows sessions and works only; inside a document it cannot yet follow a caret.
...
2. **Carets are a per-user toggle.** When document carets and diagram pointers arrive with Yjs, a setting lets the reader hide other people's carets. It defaults to on. Assumption: the toggle hides carets for the reader; it does not stop the reader's own caret from being shared.
