...
A local thread whose quote is no longer in the text has no line either. It belongs to the Page view too; the anchored margin must never hold a card at `visibility: hidden` while the header still counts it.

The header count beside the document title counts both kinds of thread and shows or hides the whole surface. Below the rail's fold width (a narrow pane or a phone) there is no margin to hold cards on their lines, so the same surface opens as a drawer over the right edge of the reading pane — the side the margin lives on — sized to the pane's own box: 20rem wide, or the whole pane once the pane is narrower than the 30rem stacked rung. `drawerFrame` in `components/comments/lib/rail-layout.ts` owns that geometry. The drawer stays closed until the count is pressed or a highlight is clicked; Escape, the close control in its header, or a click on the bare prose puts it away. Clicking any highlight, local or external, opens the drawer on that card. There is no per-thread popover any more: one thread has one place it can be read at every width.

Each thread in the rail names **Google Docs** as its origin, so **Reply…**, **Resolve**, and **Reopen** on that card are understood as external actions. **Ask agent** creates a linked local comment and opens a private prompt draft. An external card takes focus like a local one — clicking it or its highlight deepens its edge and lights the highlight in the text — and its head stays on one line, with the author truncating before the origin or the date.

### Replying to a provider thread

An answer to a Google Docs thread must land in that thread, never beside it as a new comment. Two paths keep that true:

- **From the rail.** A local thread linked to a provider thread (`externalThreadId`, the thread **Ask agent** opens) publishes every message on it as a reply into that provider thread. The provider mark on the message sends a `reply` command carrying the local `sourceMessageId`, so the receipt lights that message's mark exactly as a shared thread head's does. Only an unlinked thread publishes as a new comment.
- **From the agent.** `reply_comment` accepts a provider thread id from `read_work`. The reply stays private: it joins the linked local thread if one exists, or opens one (quoting the provider thread's text, anchored when the quote is unique), and the user publishes it from the rail. `read_work` says so beside the threads and names the document URL for the one case where the user has explicitly asked the agent to post now, which goes through `write_external_doc_comment` with action `reply`.
...
To publish a local message, select the Google Drive mark on that message. Only that message is sent. The rest of the thread stays local. New comments have no native Google Docs highlight. **Find** selects text only if the quote has one exact match. When Google has dropped a thread's quote metadata but the message still opens with the old "Quoted text:" preamble, the preamble is read back as the quote caption rather than shown raw in the body. Missing or repeated quotes leave the document selection unchanged.
