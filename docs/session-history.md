...
height changes. Find and the message navigator first mount the target turn.
...
The restored conversation is the required boot path. The entry point reads its
page in parallel with loading the renderer, then mounts only when both are ready.
Component setup converts that page synchronously into the active session before
rendering the workspace. The later history/watch reconciliation leaves existing
messages visible. Automation metadata is not required for this first conversion;
automation cards reconcile when normal hydration completes.
