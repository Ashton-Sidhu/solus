...
import type { OtelSettings, OtelSettingsSnapshot } from '../../../shared/types'
...
export interface OtelSettingsHostContext {
  serverId: string
  api: Pick<HostApi, 'otelSettingsGet' | 'otelSettingsUpdate'>
}
...
export class OtelSettingsStore {
  ...
  /** The reply carries which signals are actually exporting, which the patch
   *  alone cannot say — the host may be overridden by its environment. */
  async update(
    host: OtelSettingsHostContext,
    ...
    this.errors.set(host.serverId, null)
    try {
      const snapshot = await host.api.otelSettingsUpdate(patch)
      this.snapshots.set(host.serverId, snapshot)
      return snapshot
    } catch (error) {
      this.errors.set(
