...
export function spanDetailLabel(span: MetricsSpan): string {
  const input = span.attrs.input
  if (typeof input === 'string' && input) {
    try {
      const parsed: unknown = JSON.parse(input)
      if (parsed && typeof parsed === 'object') {
        const fields = parsed as { command?: unknown; file_path?: unknown; pattern?: unknown }
  ...
  const decision = span.attrs.decision
...
export function spanPayload(span: MetricsSpan): { label: string; text: string } | null {
  ...
  const prompt = span.attrs.prompt
