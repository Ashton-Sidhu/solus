import type { MetricsQueryResult } from '../../../../shared/observability-types'
import {
  asFiniteNumber,
  composeLabel,
  dimensionColumns,
...
function chooseMeasure(
  ...
  const picked = candidates.find((index) => result.columns[index] === chosen)
  if (picked !== undefined) return picked
  return candidates.find((index) => result.columns[index].endsWith('_ms')) ?? candidates[0]
}
...
export function trendChart(
  ...
  const grain = mark === 'line' ? bucketGrain(result.columns[timeIndex], drawn) : null
  return {
    timeColumn: result.columns[timeIndex],
    valueColumn,
    valueFormat: measureFormat(valueColumn),
    measures: candidates.map((index) => result.columns[index]),
    mark,
    seriesColumn: split ? breakdown.map((index) => result.columns[index]).join(' · ') : null,
...
export function rankingChart(
  ...
  return {
    dimensionColumn: dimensions.map((index) => result.columns[index]).join(' · '),
    valueColumn,
    valueFormat: measureFormat(valueColumn),
    measures: candidates.map((index) => result.columns[index]),
...
function rankedMeasure(result: MetricsQueryResult, candidates: number[]): string | undefined {
  ...
  return monotonic.length === 0 ? undefined : result.columns[monotonic[monotonic.length - 1]]
}
