...
<div class="mt-3.5 flex flex-wrap items-baseline gap-3">
  ...
  {#if oncancel}
    <button
      type="button"
      class="shrink-0 text-[0.71875rem] text-(--solus-text-tertiary) hover:text-(--solus-text-secondary)"
      onclick={oncancel}
    >
      Cancel
    </button>
  ...
  {#if requiresCodeInput}
    {#if submitState === "sent"}
      <span
        class="flex w-full items-center gap-2 text-[0.75rem] text-(--solus-text-tertiary)"
        role="status"
      >
    ...
    {:else}
      <form class="flex w-full items-center gap-2" onsubmit={submit}>
        <Input
          ...
          class="h-8 min-w-0 flex-1 font-mono text-[0.75rem]"
