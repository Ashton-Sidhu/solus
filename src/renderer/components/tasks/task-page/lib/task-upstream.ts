...
export function taskUpstreamState(input: {
  ...
  if (externalLink) {
    ...
    const seen = externalLink.lastSyncedAt
      ...
      : 'never synced'
    const ticket = `${externalLink.externalKey}#${externalLink.externalId}`
    return {
      provider,
      glyph: PROVIDER_GLYPHS[externalLink.provider],
      ref: `#${externalLink.externalId}`,
  ...
  return {
    provider,
    glyph: PROVIDER_GLYPHS[task.providerId],
    ref: /^\d+$/.test(task.id) ? `#${task.id}` : task.id,
    label: provider,
    tone: 'ok',
...
export interface TaskPublishTarget {
  provider: string
  /** `owner/repo`, when the status read resolved one. */
  repo: string | null
}
...
export function taskPublishTarget(input: {
  ...
  status: { provider: TaskProviderId; ok: boolean; repo?: { owner: string; repo: string } } | null
  ...
  return {
    ...
    repo: status.repo ? `${status.repo.owner}/${status.repo.repo}` : null,
  }
}

/** One recipe per tone, so the pill, its glyph and the sidebar row cannot
 *  drift apart. */
