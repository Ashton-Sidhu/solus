...
{#if files.length > 0 && anchorRect && layer.el}
  ...
  >
    <div
      class="file-menu-list rounded-[14px] bg-(--solus-popover-bg) border border-(--solus-popover-border) overflow-hidden"
      ...
      >
        {#each rows as row, i (row.file.path)}
          <button
            type="button"
            data-index={i}
            class="file-menu-row flex h-[1.625rem] w-full cursor-pointer items-center gap-2 border-0 bg-transparent px-3 text-left font-mono text-[0.75rem] font-normal text-(--solus-text-primary) hover:bg-(--solus-surface-hover)"
