<script lang="ts">
  ...
  const variantClass = $derived.by(() => {
    ...
    if (variant === 'keycap') {
      return 'h-[1.0625rem] min-w-[1.0625rem] rounded-md bg-(--solus-surface-hover) px-1.5 text-[0.65625rem] tracking-[0.03em] text-(--solus-text-tertiary) shadow-[shadow:0_0_0_0.03125rem_var(--solus-popover-border),inset_0_-0.0625rem_0_var(--solus-menu-hairline)]'
    }
    if (variant === 'accent') {
