...
import type { DocDestination, DocDiagramAsset, WorkPublishResult, WorkPullResult } from '@solus/contracts/docs'
...
export class PlanStore {
  ...
  async loadFromDisk(opts: DiskPlanLoadOptions): Promise<string> {
    ...
    const next = {
      ...
      mirroredDoc: ann?.mirroredDoc ?? existing?.mirroredDoc,
    }
    ...
    if (existing) {
      ...
      existing.mirroredDoc = next.mirroredDoc
    } else {
      this.plans[id] = {
        id,
        ...next,
  ...
  async hydrateAnnotations(planId: string, serverId?: string): Promise<void> {
    ...
    try {
      ...
      current.mirroredDoc = ann.mirroredDoc
    } catch (error) {
      console.warn('plan upstream refresh failed', planId, error)
    }
  }
  ...
  updateContent(planId: string, newContent: string): void {
    ...
    if (plan) {
      ...
      if (plan.mirroredDoc && plan.mirroredDoc.syncState === 'ok') {
        plan.mirroredDoc.syncState = 'dirty'
        this.scheduleSave(planId)
      }
    }
  }
  ...
  async publish(
    ...
    if (result.link) {
      const current = this.plans[planId]
      if (current) current.mirroredDoc = result.link
    }
    return result
  }

  async pullUpstream(planId: string): Promise<WorkPullResult> {
    const plan = this.plans[planId]
    if (!plan) return { ok: false, error: 'The plan is no longer available.' }
    const result = await this.apiForPlan(planId).pullPlanUpstream(plan.sessionId, plan.planToolUseId)
    if (!result.ok) return result
    plan.content = result.content
    plan.title = result.title
    plan.mirroredDoc = result.link
    await this.flushContentSave(planId)
    await this.persist(planId)
    return result
  }

  async refreshUpstream(planId: string): Promise<void> {
    const plan = this.plans[planId]
    if (!plan) return
    try {
      const link = await this.apiForPlan(planId).refreshPlanUpstream(plan.sessionId, plan.planToolUseId)
      const current = this.plans[planId]
      if (current && link) current.mirroredDoc = link
    } catch {}
  }

  async unlinkUpstream(planId: string): Promise<void> {
    const plan = this.plans[planId]
    if (!plan) return
    await this.apiForPlan(planId).unlinkPlanUpstream(plan.sessionId, plan.planToolUseId)
    delete plan.mirroredDoc
  }

  watchUpstream(planId: string): () => void {
    return this.upstreamWatches.watch(planId, () => this.refreshUpstream(planId))
  }
