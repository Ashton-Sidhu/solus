...
export class WindowContext {
  ...
  visible = $state(true)
  /** The window's own close/minimise/zoom controls float over the renderer's
   *  top-left corner, so the leading chrome row reserves space for them. True
   *  on the macOS editor window, where the traffic lights are native, and in
   *  the web demo, which draws that window's controls itself. */
  ...
  constructor() {
    ...
    // The editor overlays the window's controls on renderer content. Publish
    // one document flag so its shared chrome geometry switches on together.
    this.hasInsetTitlebar = (this.isMac && this.viewMode === 'editor')
      || document.documentElement.classList.contains('solus-demo')
    if (this.hasInsetTitlebar) {
