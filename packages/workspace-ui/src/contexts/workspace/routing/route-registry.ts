...
export interface RouteParams {
  ...
  review: {
    ...
    scope?: DiffScope
    filePath?: string
    /** A portable target can resolve to a host-managed checkout outside the
     * source session's project. These fields are supplied by the ready guide
     * status and stay host-scoped. */
    target?: ReviewTarget
    guideKey?: string
    cwd?: string
    serverId?: string
  ...
  /** The running UI, at a chosen viewport. Naming a page deep-links it; an
...
export interface RouteResolveContext {
  api: HostApi
  ipc: (cwd?: string) => import('@solus/contracts/types').IpcContext
}
...
/** A route id and the host it is scoped to, where the segment named one. */
interface ScopedId {
  id: string
  serverId?: string
}

function parseScopedId(segment: string): ScopedId {
...
export const ROUTES = defineRoutes({
  ...
  browser: {
    parse: (s) => {
      const { id, serverId } = parseScopedId(s)
      const params: RouteParams['browser'] = {}
      if (id) params.browserPageId = id
      if (serverId) params.serverId = serverId
      return params
    },
