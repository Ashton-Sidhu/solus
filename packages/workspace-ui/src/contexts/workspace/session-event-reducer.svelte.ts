...
export class SessionEventReducer {
  ...
  apply(sessionId: string, event: WireNormalizedEvent): void {
    ...
    if (event.type === 'text_pending') {
      session.isStreamingText = true
      return
    }
    ...
    switch (event.type) {
      case 'session_init':
        ...
        if (
          ...
          void this.deps.tasksStore.get(pendingTaskId).linkSession(
            taskSessionId,
            {
      ...
      case 'background_task_settled': {
        const msg = this.findBackgroundTaskMsg(session, event.taskId, event.toolUseId)
        if (!msg) break
        // Every tagged message records the settle, including a backgrounded Bash
        // whose tool_result landed at launch: this is the only signal it gets
        // that the work stopped, and without it the row waits forever.
        msg.backgroundTaskSettledAt = Date.now()
        ...
        if (msg.toolStatus === 'running') {
          ...
          msg.toolCompletedAt = msg.backgroundTaskSettledAt
        }
        break
      }
      ...
      case 'turn_settled':
        ...
        if (event.outcome === 'completed') {
          this.deps.playNotificationIfHidden(sessionId, 'turn_settled')
        }
        break
      ...
      case 'session_dead':
        ...
        this.resetSessionRunState(session)
        this.deps.workStreamTracker.sweep(session)
        // The run watchdog is the only emitter today and it has no exit code to
        // report — the run was already gone when it looked. Saying "exit null"
        // named a code that never existed and hid the one fact there is: the
        // session has no agent behind it any more.
        session.terminalFailure = {
          content: event.exitCode === null
            ? 'Session stopped responding — no running agent was found for it. Send a message to start a new one.'
            : `Session ended unexpectedly (exit ${event.exitCode})`,
          timestamp: Date.now(),
        }
        session.messages.push({
          id: nextMsgId(),
      ...
      case 'permission_request':
        ...
        this.deps.playNotificationIfHidden(sessionId, 'permission_request')
        break
      ...
      case 'plan': {
        ...
        this.deps.playNotificationIfHidden(sessionId, 'plan')
        break
      }
      ...
      case 'automation_saved': {
        ...
        this.deps.playNotificationIfHidden(sessionId, 'automation_saved')
        break
      }
      ...
      case 'task_created': {
        ...
        this.deps.playNotificationIfHidden(sessionId, 'task_created')
        break
      }

      case 'browser_snapshot_captured': {
        // A capture is a thing the agent saw, so it belongs in the transcript at
        // the moment it looked — not folded into whatever it says afterwards.
        session.messages.push({
          id: nextMsgId(),
          role: 'assistant',
          content: '',
      ...
      case 'agent_conversation_update': {
        ...
        if (newCard) this.deps.playNotificationIfHidden(sessionId, 'agent_conversation_new_card')
        else if (needsAttention) this.deps.playNotificationIfHidden(sessionId, 'agent_conversation_needs_attention')
        break
      }
    }
  ...
  private appendAssistantText(sessionId: string, session: Session, text: string): void {
    if (!text) return
