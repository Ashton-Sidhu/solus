...
{#snippet branchItem(branch: string, isWorktree = false)}
  <Command.Item
    value={branch}
...
{#if displayBranch}
  <Popover.Root bind:open>
    ...
    <Popover.Content
      ...
      class="menu-surface z-[10002] w-[min(19.75rem,calc(100vw-2rem))] gap-0 rounded-2xl bg-(--solus-menu-bg) p-0 text-workspace-chrome lg:text-workspace-chrome shadow-[shadow:var(--solus-menu-shadow)] ring-0 [&_.menu-row]:text-workspace-chrome [&_[data-slot=command-input]]:text-workspace-chrome"
    >
      {#if view === "worktrees"}
        <Command.Root>
      ...
      {:else}
        <Command.Root>
          ...
          <Command.List class="max-h-[288px] overflow-y-auto p-1.5">
            ...
            -->
            <Command.Group>
              {@render branchItem(displayBranch, worktreeBranches.includes(displayBranch))}
            </Command.Group>
