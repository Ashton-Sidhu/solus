<script lang="ts">
  ...
  useKeybinding(
    ...
    () => {
      if (gitCtx)
        void gitActionsFor(sourceId, session, environmentStore, pullRequests.projects).sync();
    },
    { enabled: () => focused },
  );
  useKeybinding(
    ...
    () => {
      if (gitCtx)
        void gitActionsFor(sourceId, session, environmentStore, pullRequests.projects).run(
          "commit_push",
        );
    },
    { enabled: () => focused },
