import { worktreeProjectRoot, type Automation } from '@solus/contracts/types'
...
/** The one canonical project root shown by the rail. A worktree is a checkout
 * of its base project, not a separate automation scope. */
export function automationProjectRoots(
  environmentCwd: string,
  repoRoot: string | null,
  workingDirectory: string,
): string[] {
  const path = repoRoot || environmentCwd || workingDirectory
  return path && path !== '~' ? [worktreeProjectRoot(path)] : []
}
