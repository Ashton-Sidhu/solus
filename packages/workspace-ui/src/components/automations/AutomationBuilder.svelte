<script lang="ts">
  ...
  import {
    runnableAutomationAgent,
    runnableAutomationAgents,
  ...
  const agentOptions = $derived(runnableAutomationAgents(agentContext.agents));
  ...
  // A new automation starts on an agent this host can actually run, so a
  // single-provider machine never saves one whose provider is not installed. A
  // saved automation keeps the agent it holds — it may belong to another host.
  let agentProvider = $state<AgentId>(
    initial?.action.agentProvider ??
      untrack(() => runnableAutomationAgent(agentContext.agents, undefined)),
  );
  ...
  let cwd = $state(
    initial?.action.cwd ??
      worktreeProjectRoot(
        session.activeRun?.gitContext?.repoRoot ?? session.galleryProjectPath,
      ),
  );
  ...
  const agentSelectOptions = $derived(
    (agentOptions.some((a) => a.id === agentProvider)
      ? agentOptions.map((a) => a.id)
      : // The automation holds an agent this host cannot run — a saved one from
        // another host, or nothing runnable installed here. Name it anyway: an
        // option missing from its own select reads as an empty picker.
        [...agentOptions.map((a) => a.id), agentProvider]
    ).map((id) => ({ value: id, label: agentLabel(id, agentContext.metadata) })),
  );
...
{#snippet titleBlock()}
 ...
 : 'text-muted-foreground'}">{enabled ? "Active" : "Paused"}</span
      ...
      {#if isEditing}
        <!-- The title, editable in place: same 2.125rem type as the h1 it
             replaces, so entering edit mode doesn't reflow the page. -->
        <Input
          bind:ref={titleEl}
          class="-ml-1.5 h-auto w-full rounded-lg border-0 bg-transparent px-1.5 py-0 text-2xl max-md:text-2xl font-medium leading-[1.1] text-foreground shadow-none [outline:0.0625rem_solid_transparent] transition-[outline-color,background-color] duration-120 placeholder:text-muted-foreground hover:bg-muted focus-visible:bg-transparent focus-visible:ring-0 focus-visible:[outline-color:color-mix(in_srgb,var(--solus-accent)_55%,transparent)]"
          type="text"
          bind:value={name}
          onblur={commitName}
...
{#snippet statStrip()}
  ...
  >
    <div class="flex min-w-0 flex-col gap-1">
      ...
      <span class="text-workspace-chrome font-medium">{nextRunLabel}</span>
    </div>
    <div class="flex min-w-0 flex-col gap-1">
...
{#snippet instructionsBlock()}
  <div class="mt-8.5 flex flex-col gap-4.5 [.is-laptop-display_&]:mt-7 [.is-laptop-display_&]:gap-4">
    ...
    {#if isEditing}
      <!-- One tall field at the prose column's own measure, so the writing view
           and the reading view sit on the same grid. -->
      <div class="-ml-2 flex min-h-[16rem] w-full flex-col rounded-lg bg-transparent px-2 [outline:0.0625rem_solid_transparent] transition-[outline-color,background-color] duration-120 [&_[data-testid=message-input]]:flex [&_[data-testid=message-input]]:min-h-0 [&_[data-testid=message-input]]:flex-1 [&_.cm-editor]:min-h-0 [&_.cm-editor]:flex-1 [&_.cm-scroller]:max-h-none! [&_.cm-content]:min-h-full [&_.cm-content]:![font-weight:400] [&_.cm-content]:![font-size:var(--text-sm)] [&_.cm-content]:![line-height:1.75]">
        <PromptEditor
          value={prompt}
...
>
  {@render chromeBar()}
  <!-- One page, two voices: a reading column of author-written instructions
       set like prose, and a rail of machine facts set like data. Whitespace and
       the single card edge do the dividing — no vertical rules, no card around
       the steps. Below 65rem the rail unstacks under the prose as a row of
