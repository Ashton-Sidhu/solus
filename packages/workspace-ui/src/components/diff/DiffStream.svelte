<script lang="ts">
  ...
  // header: a roomy column, a compact one (<48rem), and a stacked one (<30rem).
  const DIFF_HEADER_HEIGHT = 32;
  const DIFF_HEADER_HEIGHT_COMPACT = 30;
  const DIFF_HEADER_HEIGHT_PHONE = 28;
  // The pane's width, supplied by the panel that measures it — the same box
  // `diffTheme.ts` queries with `@container pane`. Measuring the stream's own
  // column instead would be defensible on its own, but then the painted header
  // and this metric would answer different questions and the virtualiser's row
  // heights would drift from what is drawn.
  const diffHeaderHeight = $derived(
    isStackedPane(panelWidth)
      ? DIFF_HEADER_HEIGHT_PHONE
      : isCompactPane(panelWidth)
  ...
  interface Props {
    /** The pane this stream is drawn in. Its rungs are the ones `diffTheme.ts`
     *  queries, so the virtualiser's row heights match the painted header. */
    panelWidth: number;
