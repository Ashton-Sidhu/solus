<script lang="ts">
  ...
  import { externalPrLinkCandidate } from "./lib/external-pr-link";
  ...
  const externalPr = $derived(externalPrLinkCandidate(query));
      ...
      {#if externalPr}
        ...
        >
          <svg
            ...
            aria-hidden="true"><path d="M4 1.75v7.5a3 3 0 0 0 3 3h1M4 1.75a1.5 1.5 0 1 0 0 3 1.5 1.5 0 0 0 0-3ZM10 9.25a1.5 1.5 0 1 0 0 3 1.5 1.5 0 0 0 0-3ZM10 1.75v3.5a2 2 0 0 1-2 2H7" /></svg
          >
          <span class="min-w-0 flex-1 truncate text-left">{externalPr.label}</span>
          <span class="shrink-0 text-[0.875em] text-muted-foreground opacity-70">{externalPr.meta}</span>
        </button>
      {/if}
