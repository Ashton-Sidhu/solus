...
<style>
  ...
  /* The `pane` container. A surface inside a pane learns its width from here and
     from nowhere else: the window says 1440px while this box is legally 356px.
     Named, because an unnamed `@container` in a nested pane tree resolves to
     whichever ancestor happens to be nearest, which is a coin flip.

     Declared on `.primary-column` and `.secondary-pane-content` — deliberately
     NOT on `.secondary-pane-wrap`. `container-type` applies layout containment,
     which makes the element the containing block for `position: fixed`
     descendants, exactly as the transform note below warns. The maximized
     surface *is* `.secondary-pane-content`, so declaring the container on it
     keeps `inset: 0` resolving against the window, and gives the honest answer
     in both states: the pane's width while docked, the window's while
     maximized. */
  .primary-column,
