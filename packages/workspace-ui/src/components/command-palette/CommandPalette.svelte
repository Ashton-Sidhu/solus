...
>
  <!-- `text-menu`, not the chrome rung: the palette is a decision surface like a
       menu, so it holds 14px on a laptop display instead of stepping to 12px
       (see the `html.is-laptop-display` note in index.css). -->
  <div
    class="text-menu w-[clamp(22rem,56vw,38.75rem)] max-w-[calc(100vw-3rem)] max-h-[60vh] outline-none flex flex-col rounded-2xl border-[0.0625rem] border-(--solus-popover-border) bg-(--solus-popover-bg) shadow-[shadow:var(--solus-popover-shadow),inset_0_0.0625rem_0_rgba(255,255,255,0.14),0_1.75rem_3.125rem_-1.125rem_rgba(0,0,0,0.24),0_4.375rem_8.125rem_-3.125rem_rgba(0,0,0,0.34)] [.dark_&]:shadow-[shadow:var(--solus-popover-shadow),inset_0_0.0625rem_0_rgba(255,255,255,0.06),0_1.75rem_3.125rem_-1.125rem_rgba(0,0,0,0.45),0_4.375rem_8.125rem_-3.125rem_rgba(0,0,0,0.55)] overflow-hidden origin-top"
                ...
                {#each group.items as cmd (cmd.id)}
                  <button
                    ...
                    class={cn(
                      ...
                      // The shared row steps its height, gap, radius and padding
                      // down on a laptop display. The palette holds one size at
