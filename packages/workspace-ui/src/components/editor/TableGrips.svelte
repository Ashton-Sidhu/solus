<script lang="ts">
  ...
  $effect(() => {
    ...
    const onBlur = () => {
      // Tiptap can emit blur while Svelte is evaluating the portal markup.
      // Defer the reactive writes so they do not run inside that evaluation.
      queueMicrotask(() => {
        if (!isActive || ed.isFocused) return;
        table = null;
        geometry = null;
        selected = null;
      });
