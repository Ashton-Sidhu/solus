...
import { serializeReferenceToken, type PlanReferenceToken } from './reference-tokens'
...
function planToken(attrs: Partial<PlanRefAttrs> | undefined): PlanReferenceToken {
  return {
    kind: 'plan',
    planId: attrs?.planId ?? '',
...
export const PlanRefExtension = Node.create({
  ...
  renderMarkdown(node) {
    return serializeReferenceToken(planToken(node.attrs))
  },
