...
import { serializeReferenceToken, type SlashReferenceToken } from './reference-tokens'
...
function slashToken(attrs: Partial<SlashRefAttrs> | undefined): SlashReferenceToken {
  return { kind: 'slash', command: attrs?.command ?? '' }
}
...
export const SlashRefExtension = Node.create({
  ...
  renderMarkdown(node) {
    return serializeReferenceToken(slashToken(node.attrs))
  },
