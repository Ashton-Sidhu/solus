...
export interface RowInput {
  ...
  /** What the current conversation's task is linked to, newest link first. */
  linked: MenuItem[];
  /** That task's short name, shown as the band's hint. */
  linkedLabel: string;
...
/** No query: the six categories, then what this conversation's task is linked
 *  to, then what you last touched across all of them. The categories keep the
 *  top of the menu because they are the fixed shape a user navigates by. */
function referenceBrowseRows(input: RowInput): MenuRow[] {
  ...
  if (input.linked.length > 0) {
    rows.push({
      ...
      key: "linked",
      label: "Linked",
      // The task's name, and the cap when there is more than the band shows.
      hint:
        input.linked.length > MAX_LINKED
          ? `${input.linkedLabel} · ${bandHint(input.linked.length, MAX_LINKED)}`
          : input.linkedLabel,
    });
    rows.push(
      ...input.linked
...
/**
 * The opening prompt drawn under the list for the selected session.
 *
 * Offered only inside the sessions category, where every row is a session and
 * the block is plainly about the highlighted one. In a mixed band — browse,
 * Recent, best matches — it sits rows away from its subject and reads as a
 * stray paragraph of someone else's conversation.
 */
export function sessionPreviewFor(
  trigger: Trigger,
  row: SelectableRow | null,
): string {
  if (trigger.char !== "#" || trigger.path[0] !== "session") return "";
  if (row?.type !== "item" || row.item.refKind !== "session") return "";
