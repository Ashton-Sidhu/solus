...
>
  ...
  >
    <header class="flex items-center gap-2 border-b border-(--solus-container-border) px-4 py-3">
      <GraphIcon size={16} class="text-(--solus-accent)" />
    ...
    <div class="max-h-80 overflow-y-auto p-2">
      ...
      {:else}
        {#each filtered as choice, index (choice.workId)}
          ...
          >
            <span class="flex size-8 shrink-0 items-center justify-center rounded-lg bg-(--solus-accent-light) text-(--solus-accent)"><GraphIcon size={15} /></span>
