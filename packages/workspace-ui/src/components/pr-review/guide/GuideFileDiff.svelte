<script lang="ts">
  ...
  const localMoveAnalysis = $derived(detectMovedBlocks(analyzedFiles));
...
<Button
  type="button"
  variant="ghost"
  class="flex min-h-10 w-full cursor-pointer items-center gap-2 px-3 py-2 text-left text-xs text-(--solus-accent) transition-[background-color,scale] duration-150 hover:bg-(--solus-surface-hover) active:scale-[0.96] {autoCollapse &&
  !expanded
    ? ''
    : 'hidden'}"
...
<Button
  type="button"
  variant="ghost"
  class="flex min-h-10 w-full cursor-pointer items-center gap-2 px-3 py-2 text-left text-xs text-(--solus-accent) transition-[background-color,scale] duration-150 hover:bg-(--solus-surface-hover) active:scale-[0.96] {formatOnlyCollapsed &&
  (!autoCollapse || expanded)
    ? ''
    : 'hidden'}"
