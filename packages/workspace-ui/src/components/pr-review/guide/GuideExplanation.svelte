...
<div class="flex flex-col gap-3.5">
  ...
  {#each questions as question (question)}
    ...
    >
      ...
      <p class="leading-relaxed text-(--solus-text-primary)">{question}</p>
    </div>
  {/each}
  ...
  {#if detailRecords.length > 0}
    <details class="group rounded-lg bg-(--solus-art-surface) px-3 py-2">
      <summary
        class="flex cursor-pointer list-none items-center gap-1.5 font-medium text-(--solus-text-tertiary) select-none"
      >
        <span
      ...
      <div class="detail-body mt-2.5 flex flex-col gap-3">
        {#each detailRecords as record (record.id)}
          <div class="flex flex-col gap-2.5">
            <p class="font-medium text-(--solus-text-primary)">{record.title}</p>
            {#each DETAIL_FIELDS as field (field.key)}
              {@const detail = record[field.key]}
              {#if detail}
                <div class="flex flex-col gap-1">
                  <p class="text-chrome-shelf font-medium text-(--solus-text-tertiary) uppercase">
                    {field.label}
                  </p>
                  <div class="prose-cloud prose-reading prose-review-guide font-secondary text-(--solus-text-secondary)">
