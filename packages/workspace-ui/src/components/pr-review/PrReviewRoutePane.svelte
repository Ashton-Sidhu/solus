<script lang="ts">
  ...
  // The scope this link names: the PR's own directory when it carries one,
  // else the workspace's.
  const routeCtx = $derived(
    params.cwd ? session.ctxForDirectory(params.cwd) : session.ctx,
  );
  // A parsed link can omit the host; then the pane keeps the same host the
  // route's `resolve` used — the workspace's for this PR's directory.
  const api = $derived(
    params.serverId
      ? serverConnections.apiFor(params.serverId)
      : session.apiForContext(routeCtx),
  );
  const serverId = $derived(
    params.serverId ?? session.serverIdForContext(routeCtx),
  );

  async function refreshTarget(): Promise<void> {
    const next = await api.prOpenReview(routeCtx, params.number);
