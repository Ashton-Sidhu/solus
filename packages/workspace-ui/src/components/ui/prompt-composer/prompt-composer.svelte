...
<!-- The input bar's card, at the same measurements (see EditorInputCard): a
     ...
     labels. -->
<div
  class="@container/composer flex flex-col rounded-2xl bg-(--solus-input-pill-bg) px-3 pb-3 transition-[box-shadow] duration-[180ms] {focused
...
>
  ...
  <!-- Same geometry and the same rung as the input bar's toolbar row
       (InputToolbar.svelte). This row used to scale itself with the text
       preference, the last of the two sites that did. That is the wrong axis
       twice: scaling the row put it in a different
       coordinate space from the `composer` container its ladder queries, and it
       grew chrome with a preference whose purpose is to grow prose *relative*
       to chrome (ADR-0013). The chrome rung declares the size instead. -->
  <div class="flex items-center gap-2 w-full text-workspace-chrome [&_button]:text-[length:inherit]">
    {#if collapsible}
      <TooltipUI.Root>
        <TooltipUI.Trigger>
          {#snippet child({ props: tooltipProps })}
