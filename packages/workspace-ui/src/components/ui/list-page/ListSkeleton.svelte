<script lang="ts">
   ...
   *
   *  One group, drawn at one weight. Previewing a second, dimmer group read as
   *  two competing skeletons rather than as one list still arriving, so the
   *  silhouette states the first group only and lets later groups arrive with
   *  the data. Every list page draws the same shape. */
  interface Props {
