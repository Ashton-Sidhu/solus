<script lang="ts" generics="T extends string">
  ...
  const segBtnClass = $derived(
    bar && compact
      ...
      : bar
      ? "inline-flex h-full cursor-pointer items-center gap-1.5 whitespace-nowrap rounded-md border-0 px-2.5 text-[length:inherit] transition-[background-color,color] duration-100 ease-in-out focus-visible:outline-2 focus-visible:outline-offset-1 focus-visible:outline-[color-mix(in_srgb,var(--solus-accent)_50%,transparent)] @max-[16rem]:gap-1 @max-[16rem]:px-1.5"
      ...
      {#if opt.count !== undefined}
        <span
          class="tabular-nums {bar && compact
            ? 'hidden'
            : bar
            ? ' text-xs opacity-65 @max-[16rem]:hidden'
            : active
