<script lang="ts">
  ...
  let composerEl: ReturnType<typeof CommentEditor> | null = $state(null);
  let started = false;
  let disposed = false;
  ...
  const reviewServerId = $derived(pullRequests.view.reviewModeServerId);
  ...
  function findSummary(number: number): PullRequest | null {
    ...
    const project = pullRequests.projects.at(reviewServerId, projectScopeOf(ctx.session));
    return project?.prFor(number)
      ?? pullRequests.needsReview.itemsFor(reviewServerId ?? '', ctx).find((item) => item.number === number)
      ?? null;
  }
...
<section class="flex h-full min-h-0 flex-col bg-(--solus-container-bg) antialiased" aria-label="Review mode">
 ...
 : 'text-(--solus-accent) ring-(--solus-accent-border)'}">
            ...
            {#if source}
              <PrChecksChip
                summary={pullRequests.checks.summaryFor(reviewServerId, postingContext ?? session.ctx, source.number)}
                headSha={source.headSha}
                loadFailed={pullRequests.checks.loadFailedFor(reviewServerId, postingContext ?? session.ctx)}
              />
            {/if}
 ...
 : 'text-(--solus-text-tertiary) hover:text-(--solus-text-primary)'}"
        ...
        {#if composerOutcome}
          <div class="flex shrink-0 items-end gap-1 border-t border-(--solus-accent-border) bg-(--solus-accent-light) px-3 py-2">
            <div class="min-w-0 flex-1">
              <span class="mb-1 block text-xs font-medium text-(--solus-text-secondary)">
                {composerOutcome === "changes_requested" ? "Why are you requesting changes?" : "Leave a review comment"}
              </span>
              <CommentEditor
                bind:this={composerEl}
                value={composerBody}
                onValueChange={(value) => (composerBody = value)}
                maxHeight={112}
                class="min-h-14 rounded-lg border border-(--solus-accent-border) bg-(--solus-container-bg) px-3 py-2 text-xs font-normal focus-within:border-(--solus-accent) focus-within:ring-2 focus-within:ring-(--solus-accent-border)"
