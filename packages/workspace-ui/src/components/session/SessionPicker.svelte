<script lang="ts">
  ...
  import { worktreeProjectRoot, type SessionMeta } from "@solus/contracts/types";
  ...
  // A picker row is read with a pointer on a desktop and tapped with a thumb on
  // a phone, and 46px is neither a comfortable target nor enough room for a
  // title at reading size. The picker is a portalled overlay sized against the
  // window, so the window is the honest thing to ask here.
  const ENTRY_HEIGHT = $derived(runtime.isMobileViewport ? 62 : 46);
  const HEADER_HEIGHT = $derived(runtime.isMobileViewport ? 30 : 26);
...
{#snippet pickerHeader()}
  <div
    class="relative flex flex-shrink-0 items-center gap-2.5 px-[1.125rem] after:absolute after:bottom-0 after:left-[1.125rem] after:right-[1.125rem] after:h-px after:content-[''] max-md:h-auto max-md:flex-col max-md:items-stretch max-md:gap-0 max-md:border-b max-md:border-[var(--hairline)] max-md:px-4 max-md:pt-1 max-md:pb-3 max-md:after:hidden
 {inline ? 'h-11' : 'h-12 max-md:h-auto'}
 {historyLoading
 ...
 : 'after:bg-[var(--solus-popover-border)] after:opacity-35'}"
  >
    <!-- The field is a rule under a row of chrome on a desktop and a raised card
         on a phone, which is the same shape every other search on the phone
         takes. `display: contents` above the rung leaves the desktop row exactly
         as it was. -->
    <div
    ...
    <MagnifyingGlassIcon
      size={14}
      class="flex-shrink-0 text-(--solus-text-tertiary) max-md:size-4"
    />
    <Input
      bind:ref={searchEl}
      bind:value={query}
        ...
        >
          {#snippet item({ style, index })}
            ...
            <div {style}>
              {#if row.kind === "header"}
                <div
                  class="flex h-full items-end px-5 pb-1 text-xs font-medium uppercase tracking-[0.06em] text-[var(--solus-text-tertiary)] max-md:px-6 max-md:pb-1.5 max-md:tracking-[0.12em]"
                >
                  {row.label}
                </div>
              {:else}
