  ...
  <nav class="flex flex-col gap-0.5 w-60 shrink-0 border-r border-r-border pr-3" aria-label="Projects">
    ...
    {#each projects as project (project.path)}
      ...
      <div class="group flex items-center gap-1 w-full rounded-md border min-w-0 [transition:background_0.15s_ease,color_0.15s_ease,border-color_0.15s_ease]
        ...
        <button
          type="button"
          class="flex items-center gap-2.5 flex-1 min-w-0 overflow-hidden text-left py-2 px-2.5 border-none bg-transparent text-inherit cursor-pointer"
          aria-current={active}
        ...
        >
          ...
          <span class="flex flex-col min-w-0 gap-px">
            <span class="text-workspace-chrome {active ? 'font-medium' : 'font-normal'} whitespace-nowrap overflow-hidden text-ellipsis">{project.folderName || folderName(project.path)}</span>
