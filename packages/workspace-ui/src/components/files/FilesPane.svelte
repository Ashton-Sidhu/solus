<script lang="ts">
  ...
  // The tree and the file it opens are two columns; below 30rem of *pane* they
  // cannot both be read, so the split turns vertical. `panelWidth` is this
  // pane's own box, already bound below — the window it sits in was never the
  // question, and a 380px files pane on a 27" display used to get the
  // two-column answer.
  ...
  async function mutate(mutation: ProjectFileMutation): Promise<boolean> {
    const result = await api.mutateProjectFile(ctx, {
      cwd: root || cwd,
      mutation,
    });
  ...
  function treeComposition(): FileTreeCompositionOptions {
    return {
      contextMenu: {
        enabled: true,
        triggerMode: "both",
        buttonVisibility: runtime.isTouchDevice ? "always" : "when-needed",
        onOpen: (item, context) =>
