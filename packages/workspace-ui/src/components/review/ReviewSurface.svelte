<script lang="ts">
  ...
  let {
    ...
    target?: ReviewTarget;
    guideKeyOverride?: string;
    checkoutRepoRoot?: string;
  ...
  const targetPullRequest = $derived(target?.kind === "pr" ? target : null);
  const targetLabel = $derived(
    targetPullRequest
      ? `${targetPullRequest.owner}/${targetPullRequest.repo} PR #${targetPullRequest.number}`
      : undefined,
  );
  const getCtx = () => checkoutRepoRoot
    ? session.ctxForEnvironment(
        checkoutRepoRoot,
        {
          branch: targetLabel ?? null,
          targetBranch: targetPullRequest?.baseSha ?? "HEAD",
          repoRoot: checkoutRepoRoot,
          worktreePath: checkoutRepoRoot,
        },
        sourceTabId,
      )
    : session.ctxForEnvironment(environment.cwd, environment.checkout, sourceTabId);
  const getApi = () => serverIdOverride
    ? serverConnections.apiFor(serverIdOverride)
    : session.apiFor(sourceTabId);
  const getServerId = () => serverIdOverride ?? session.serverIdFor(sourceTabId);
  ...
  // Kept whole, not reduced to the base it names: the guide asks the same
  // question (for its own staleness check), and one answer read twice is one
  // round trip instead of two on the way to the first painted hunk.
  let reviewContext = $state<ReviewContext | null>(null);
  // Separate from the context itself: a checkout with no resolvable context is
  // an answer — read the working tree — and null alone cannot tell that apart
  // from not having asked yet.
  let hasResolvedReviewContext = $state(false);
  const branchScope = $derived<DiffScope | null>(
    !hasResolvedReviewContext
      ? null
      : reviewContext?.baseSha
        ? { kind: "pr", baseSha: reviewContext.baseSha }
        : { kind: "working-tree" },
  );
  let branchScopeError = $state<string | null>(null);
  $effect(() => {
    if (scope || target?.kind === "pr") return;
    if (!checkoutRepoRoot && !environment.repoRoot) return;
    const api = getApi();
    const ctx = getCtx();
    let current = true;
    void api
      ...
      .then((resolved) => {
        if (!current) return;
        reviewContext = resolved;
        hasResolvedReviewContext = true;
      })
      .catch((error) => {
        if (!current) return;
  ...
  const targetDiffScope = $derived<DiffScope | null>(
  ...
  const loader = new GuideLoader({
    ...
    // fetching the same bytes beside it.
    getHostPatch: (guideDiffScope) =>
      diffScope?.kind === "pr" &&
      diffScope.baseSha === guideDiffScope.baseSha &&
      !diffScope.ownDeltaBaseSha
        ? diffPanelRef?.currentPatch() ?? ""
        : null,
  });
  // A durable PR reference can omit revisions. In that case the cached guide
  // is the authority for its recorded base SHA; never resolve the managed
  // checkout against HEAD, which produces an empty HEAD..HEAD patch.
  const diffScope = $derived<DiffScope | null>(
    scope ?? targetDiffScope ?? (targetPullRequest ? loader.diffScope : branchScope),
  );
  ...
  // own in-flight call.
  const guideStatus = $derived(reviewGuideStore.statusFor(getServerId(), guideIdentity));
  ...
  $effect(() => {
    ...
    reviewGuideStore.markOpened(getServerId(), guideIdentity);
  });

  const guideUnread = $derived(
    reviewGuideStore.indicatorStatusFor(getServerId(), guideIdentity)?.status === "ready",
  );
...
{#snippet mapView(files: FileDiffMetadata[])}
  <DiffHeatMap
    ...
    repoRoot={checkoutRepoRoot ?? environment.worktreePath ?? environment.repoRoot ?? environment.cwd}
...
{:else if diffScope}
  <DiffPanel
    ...
    worktreePath={checkoutRepoRoot ?? sourceSession?.run.gitContext?.worktreePath ??
    ...
    feedbackToNewSession={target?.kind === "pr"}
    feedbackSessionTarget={targetPullRequest && checkoutRepoRoot
      ? {
          workingDirectory: checkoutRepoRoot,
          gitContext: {
            branch: null,
            detachedHeadSha: targetPullRequest.headSha,
            targetBranch: targetPullRequest.baseSha ?? "HEAD",
            repoRoot: checkoutRepoRoot,
            worktreePath: checkoutRepoRoot,
          },
