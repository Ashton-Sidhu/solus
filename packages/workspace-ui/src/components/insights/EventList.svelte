<script lang="ts">
  ...
  import type { MetricsValue } from '@solus/contracts/observability-types'
  ...
  import { asFiniteNumber, displayCell } from './lib/result-columns'
  ...
  function cellText(column: EventColumn, value: MetricsValue): string {
    ...
    if (numeric !== null && column.format === 'time') {
      return spansDays ? formatDayClock(numeric) : formatClock(numeric);
    }
    if (numeric !== null && column.format === 'duration') return formatDuration(numeric);
