...
/** Replaces only the range predicate the NL compiler was instructed to emit.
 *  Other dates and user-authored predicates are not touched. A null result
 *  means the question chose its own period or the compiler did not apply the
 *  managed window, so the caller must not guess at a rewrite. */
export function replaceRangeCondition(
  sql: string,
  previous: TimeRange,
  next: TimeRange,
): string | null {
  const column = String.raw`((?:[a-z_][\w]*\.)?started_at)`
  const pattern = previous.kind === 'relative'
    ? new RegExp(
      String.raw`${column}\s*>\s*\(\s*strftime\(\s*'%s'\s*,\s*'now'\s*\)\s*\*\s*1000\s*-\s*${Math.round(previous.ms)}\s*\)`,
      'gi',
    )
    : new RegExp(
      String.raw`${column}\s*>=\s*${Math.round(previous.from)}\s+and\s+\1\s*<\s*${Math.round(previous.to)}`,
      'gi',
    )
  let replaced = false
  const rewritten = sql.replace(pattern, (_condition, matchedColumn: string) => {
    replaced = true
    return rangeCondition(next, matchedColumn)
  })
