...
/** The one attribute read that still decodes. Every other field here is a
 *  number or a flag, where a wrong type renders as a wrong number; these three
 *  are printed as someone's words, and a host sending a number where the
 *  registry declares text must not have it read back as the user's message. */
function paneText(value: string | undefined): string {
...
export function turnTranscript(root: MetricsSpan | null): TranscriptPane[] {
  ...
  const systemChars = root?.attrs.systemPromptChars ?? null
  ...
  return [
    {
      ...
      text: paneText(root?.attrs.prompt),
      meta: inputTokens == null ? null : `${formatTokens(inputTokens)} in`,
      emptyNote: 'This turn recorded no prompt text.',
      truncated: root?.attrs.promptTruncated === true,
      chars: root?.attrs.promptChars ?? null,
    },
    {
      role: 'response',
      label: answeredBy,
      ...
      text: paneText(root?.attrs.response),
      ...
      truncated: root?.attrs.responseTruncated === true,
      chars: root?.attrs.responseChars ?? null,
    },
    {
      role: 'system',
      label: 'System prompt',
      ...
      text: paneText(root?.attrs.systemPrompt),
      ...
      truncated: root?.attrs.systemPromptTruncated === true,
      chars: systemChars,
    },
  ]
}
