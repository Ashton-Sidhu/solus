...
// Result rendering → which view answers the query (docs/plans/observability.md).
//
// The server declares the grain (`sourceView`); this module maps it to one of
// the five locked renderings: a turn listing, an event listing (span-grained rows
...
/** Which rendering answers a query (docs/plans/observability.md). */
export type ResultRendering =
  | { rendering: 'turns' }
  | { rendering: 'events'; view: string; kind: string }
  | { rendering: 'trend'; trend: Trend }
  | { rendering: 'ranking'; ranking: Ranking }
  | { rendering: 'table' }
...
export interface EventColumn {
  name: string
  format: CellFormat
  /** Right-aligned tabular numbers: the registry's declared type where the
   *  column has one, read off the cells where the query invented it. */
  numeric: boolean
}
...
export function resultRendering(
  ...
  if (!result) return { rendering: 'table' }
  if (isTurnResult(result)) return { rendering: 'turns' }
  const view = result.sourceView
  if (view !== undefined && view !== 'turns' && columnIndex(result, 'started_at') !== -1) {
    return { rendering: 'events', view, kind: eventKind(result, schema, view) }
  }
  const trend = trendChart(result, measure)
  if (trend && isReadableTrend(trend)) return { rendering: 'trend', trend }
  const ranking = rankingChart(result, measure)
  if (ranking) return { rendering: 'ranking', ranking }
...
function eventKind(
  ...
  const kindIndex = columnIndex(result, 'kind')
  if (kindIndex !== -1) {
...
/** How a cell prints. The registry's declared type answers it where there is
 *  one; a column the query invented falls back to its name's suffix. */
function columnFormat(column: MetricsResultColumn): CellFormat {
  if (column.name === 'started_at' || column.name === 'ended_at') return 'time'
  if (column.name === 'status') return 'status'
  if (column.type === 'duration' || column.name.endsWith('_ms')) return 'duration'
  return 'raw'
}
 ...
 *
 *  Each column states its own registry type, so nothing here reads the schema. */
export function toEventTable(result: MetricsQueryResult): EventTable {
  const index = new Map(result.columns.map((column, at) => [column.name, at]))
  ...
  const displayed = result.columns.filter(
    ({ name }) => !EVENT_HIDDEN_COLUMNS.has(name) && (name !== 'kind' || kindValues.size > 1),
  )
  const displayNames = displayed.map((column) => column.name)
  const numeric = numericColumns(result)
  const columns: EventColumn[] = displayed.map((column) => ({
    name: column.name,
    format: columnFormat(column),
    numeric: numeric[index.get(column.name) ?? -1] === true,
  }))
