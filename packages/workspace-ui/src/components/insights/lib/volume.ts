...
export interface VolumeBucket {
  ...
  /** Spend across the rows that report one. */
  costUsd: number
  /** How many of `total` reported a cost at all. A model without a known price
   *  can leave a bucket partly costed; the tooltip says which fraction rather
   *  than letting the sum read as the whole bucket's cost. */
  costedCount: number
}
...
export function bucketPoints(
  ...
  for (const point of points) {
    ...
    if (point.costUsd != null) {
      buckets[index].costUsd += point.costUsd
      buckets[index].costedCount += 1
    }
...
/** Rebins the server's bounded aggregate series to the number of bars the
 * current plot can display. Counts and spend remain exact because only
 * aggregate buckets are combined; raw turns never cross the host boundary. */
export function rebucketVolume(
  source: MetricsTurnVolumeBucket[],
  from: number,
  to: number,
  count: number,
): VolumeBucket[] {
  ...
  for (const item of source) {
    const midpoint = item.at + (item.endAt - item.at) / 2
    const index = Math.min(count - 1, Math.max(0, Math.floor((midpoint - from) / width)))
    const bucket = buckets[index]
    bucket.total += item.total
    bucket.claudeCode += item.claudeCode
    bucket.codex += item.codex
    bucket.unknownProvider += item.unknownProvider
    bucket.costUsd += item.costUsd
    bucket.costedCount += item.costedCount
  }
  return buckets
}

/** The rungs a cost axis is allowed to top out on, per power of ten. */
const COST_AXIS_STEPS = [1, 2, 5]

/**
 * The top of the spend axis: the nearest round value at or above the dearest
 * bucket.
 *
 * The cost line shares the bars' plot, so its own domain has to be nameable in
 * three labels — `$0.50` rather than `$0.437` — and it has to leave the peak
 * inside the plot rather than pinned to the ceiling. Zero means no counted row
 * reported a cost, which is the signal to draw no line and no axis at all: an
 * event listing records spend nowhere, and a flat line along the baseline reads
 * as "this work was free".
 */
export function costAxisMax(buckets: VolumeBucket[]): number {
  const peak = Math.max(0, ...buckets.map((bucket) => bucket.costUsd))
  if (peak <= 0) return 0
  const magnitude = 10 ** Math.floor(Math.log10(peak))
  const step = COST_AXIS_STEPS.find((rung) => peak <= rung * magnitude)
  return step ? step * magnitude : 10 * magnitude
}
