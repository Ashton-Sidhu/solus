import type { MetricsFieldType, MetricsQuerySpec, MetricsResultColumn } from '@solus/contracts/observability-types'
...
/** Bounded context rows used by non-paged fallback queries. The normal turn
 * listing uses `metricsTurnPage` and does not truncate its selected dataset. */
...
/** The type the `turns` view declares for each selected column. A live result
 *  carries these from the field registry; stating them here is what lets the
 *  demo serve a listing identical to a real one. `satisfies` makes a newly
 *  selected column without a declared type a compile error. */
const TURN_COLUMN_TYPES = {
  span_id: 'string',
  trace_id: 'string',
  session_id: 'string',
  started_at: 'number',
...
} satisfies Record<(typeof TURN_SELECT_COLUMNS)[number], MetricsFieldType>

/** The default explore query's result columns, typed as the server types them. */
export const TURN_LISTING_COLUMNS: MetricsResultColumn[] = TURN_SELECT_COLUMNS.map((name) => ({
  name,
  type: TURN_COLUMN_TYPES[name],
}))
