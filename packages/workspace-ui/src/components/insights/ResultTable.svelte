<script lang="ts">
  ...
  import { displayCell, numericColumns as numericColumnsOf } from './lib/result-columns'
  ...
  const numericColumns = $derived(
    numericColumnsOf(result),
  )
  const gridColumns = $derived(
    result.columns.map((column, index) => ({ name: column.name, numeric: numericColumns[index] })),
  )
  const data = $derived(
            ...
            {#each headerGroup.headers as header (header.id)}
              ...
              >
                {#if !header.isPlaceholder}
                  ...
                  >
                    <span class="truncate">{result.columns[index].name}</span>
                    <DataTableSortIcon direction={sorted} />
                  </button>
                {/if}
                <DataTableResizeHandle
                  columnLabel={result.columns[index].name}
