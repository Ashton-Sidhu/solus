<script lang="ts">
  ...
  /** Which of the five result shapes this answer renders as. */
  const shape = $derived(resultShape(store.result, store.schema, measure));
  ...
  const listsRows = $derived(shape.shape === "turns" || shape.shape === "events");
  ...
  const chartCountLabel = $derived(
    answerPoints.length > 0 && shape.shape === "events"
      ? labelForKind(shape.kind) || shape.view.replace(/_/g, " ")
      : "Turns",
  );
  ...
  function resetQuery(): void {
    void store.runGenerated({ kind: "explore" });
  }
  ...
  const railItems = $derived(
    shape.shape === "events" && eventTable
      ? railItemsFromEvents(eventTable.columns, visibleEventRows, shape.kind)
  ...
  const listLabel = $derived(
    shape.shape === "events"
      ? "Events"
      : shape.shape === "turns" || !store.result
        ? "Turns"
        : "Results",
  );
...
>
  ...
  >
    ...
    <!-- The console owns the question's controls — the range lives on it. The
         header keeps only the way back to the default question. -->
    {#if !splitList}
      <button
        type="button"
