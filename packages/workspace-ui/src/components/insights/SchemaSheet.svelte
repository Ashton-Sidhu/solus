    ...
    {:else}
      <div class="flex min-h-0 flex-1 flex-col md:flex-row">
        ...
        <!-- The pane: the table choice, one table's columns, or the search.
             Its `@container` stays unnamed on purpose: it is the sheet's own
             scroll body, queried only by the sheet's own rows. Naming it would
             imply it is one of the three workspace containers, which it is not. -->
        <div
          bind:this={paneEl}
