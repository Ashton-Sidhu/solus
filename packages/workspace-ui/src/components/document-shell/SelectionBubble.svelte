<script lang="ts">
  ...
  // On a phone the bar carries Comment, Suggest and Copy, in that order —
  ...
  async function copySelection() {
    ...
    await navigator.clipboard?.writeText(state.doc.textBetween(from, to, "\n"));
  }
...
<style>
  /* Touch: 44px tall, opaque, pill — three actions a thumb can hit rather than
     six 26px glyph buttons a stylus would want. */
  .doc-bubble--touch {
    height: 2.75rem;
    padding: 0 0.25rem;
