<script lang="ts">
  ...
  import { resolveAnswer, summarizeAnswered } from './lib/answered-question';
  ...
  const summary = $derived(summarizeAnswered(questions));
  // Nothing structured survived, so the row says what it has instead of a pick.
  const fallbackTarget = $derived(
    message.questionResult ?? (message.toolStatus === 'error' ? 'did not complete' : 'no answer recorded'),
  );
...
<!-- The resolved twin of QuestionCard, and history rather than an interrupt: it
     folds to the same one-line activity row every finished step in the turn
     uses, and opens to the full record of what was asked and picked. -->
<section
  class="my-2 min-w-0"
