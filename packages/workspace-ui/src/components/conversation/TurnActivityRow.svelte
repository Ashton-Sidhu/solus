<script lang="ts">
  ...
  interface Props {
     ...
     *  This row owns the spinner whenever the turn's tail is not a tool group —
     *  which is the usual shape once the agent has replied and gone quiet. */
  ...
  // The background wait is this row's last answer, not its first: a running
  // foreground tool is the more immediate thing to say. So it speaks in the
  // gaps — exactly where the row used to claim it was planning.
  const showsBackgroundWait = $derived(
    live && !runningTool && backgroundWait !== null,
  );
