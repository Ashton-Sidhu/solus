...
>
  ...
  {#if verdict}
    <div
      class="px-[1.0625rem] pt-3.5 pointer-fine:[.is-laptop-display_&]:px-3.5 pointer-fine:[.is-laptop-display_&]:pt-3"
    >
  ...
  {#if reportPreview.headings.length > 0}
    <!-- An index, not a second report: one row per section, each a single line,
         so the rows read as a rail the eye runs down rather than as prose. -->
    <div
      class="flex flex-col px-[1.0625rem] pt-[0.9375rem] pointer-fine:[.is-laptop-display_&]:px-3.5 pointer-fine:[.is-laptop-display_&]:pt-3"
      data-testid="subagent-findings"
    >
      {#each reportPreview.headings as heading, i (i)}
        <div
          class="truncate border-t border-[color-mix(in_oklch,var(--foreground)_7%,transparent)] py-2 font-medium text-(--muted-foreground) uppercase opacity-70 pointer-fine:[.is-laptop-display_&]:py-1.5"
        >
          {heading}
        </div>
      {/each}
      {#if reportPreview.more > 0}
        <div
          class="border-t border-[color-mix(in_oklch,var(--foreground)_7%,transparent)] py-2 text-(--muted-foreground) pointer-fine:[.is-laptop-display_&]:py-1.5"
        >
  ...
  {#if figures.length > 0}
    ...
    >
      {#each figures as item (item.label)}
        <div
          class="flex items-baseline gap-3.5 border-t border-[color-mix(in_oklch,var(--foreground)_7%,transparent)] py-2 pointer-fine:[.is-laptop-display_&]:py-1.5"
        >
  ...
  <div
    class="mt-3.5 flex items-center gap-2.5 border-t border-[color-mix(in_oklch,var(--foreground)_7%,transparent)] px-[1.0625rem] pt-2.5 pb-3 pointer-fine:[.is-laptop-display_&]:mt-3 pointer-fine:[.is-laptop-display_&]:px-3.5 pointer-fine:[.is-laptop-display_&]:pt-2 pointer-fine:[.is-laptop-display_&]:pb-2.5"
  >
    <span
