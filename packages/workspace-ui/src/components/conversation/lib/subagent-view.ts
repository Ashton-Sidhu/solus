...
/** A heading is set as chrome — uppercase, one line — so its markdown has to be
 *  resolved to text first. Left in, a code span prints its own backticks and a
 *  link prints its URL. Underscores are not stripped: `snake_case` in a heading
 *  is commoner than underscore emphasis, and gutting it is the worse failure. */
const HEADING_LINK = /\[([^\]]+)\]\([^)]*\)/g
const HEADING_EMPHASIS = /(\*\*|\*)(?=\S)([\s\S]*?\S)\1/g
/** The agent's own numbering. Both surfaces count the sections themselves — the
 *  pane in its margin, the card by position — so a kept ordinal reads as a
 *  second counter that disagrees whenever the agent skips or restarts one. */
const HEADING_ORDINAL = /^\d+[.)]\s+/

export function headingText(raw: string): string {
  let text = decodeHtmlEntities(raw).replace(HEADING_LINK, '$1').replace(/`+/g, '')
  // `***both***` sheds one marker per pass, so run until the text settles.
  for (let pass = 0; pass < 3; pass++) {
    const next = text.replace(HEADING_EMPHASIS, '$2')
    if (next === text) break
    text = next
  }
  return text.replace(HEADING_ORDINAL, '').trim()
}
