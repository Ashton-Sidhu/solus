  ...
  async run(
    ...
    try {
      ...
      if (pullRequest.status !== 'skipped') {
        ...
        // Creating a pull request is the one moment Solus knows about it before
        // any provider list does. Hand it to the store so the git rail, the
        // sidebar chip and the task row can name it now rather than after the
        // next refresh.
        this.pullRequests
          .get(api, this.session.serverIdFor(this.sourceId), target.ctx)
          .absorbCreated(pullRequest)
...
export function gitActionsFor(
  ...
  if (!existing) {
    existing = new GitActions(session, sourceId, environmentStore, pullRequests)
