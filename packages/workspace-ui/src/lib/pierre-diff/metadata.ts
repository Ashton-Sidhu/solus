...
const SECONDARY_SEED = 0x9e3779b9
const SECONDARY_MULTIPLIER = 0x85ebca6b

function fnv1a(text: string, seed: number, multiplier: number): number {
  let hash = seed >>> 0
...
/**
 * A content fingerprint for one patch or one `diff --git` chunk.
 *
 * @pierre's worker pool keys its rendered-AST cache by `cacheKey` **alone**
 * (`WorkerPoolManager.getDiffResultCache`), and `DiffHunksRenderer.renderDiff`
 * installs whatever that lookup returns into its render cache. So a cacheKey
 * has to identify the *rendered rows*, not just the file.
 *
 * Parsing without a key is not the safe option it looks like: `hydratePartialDiff`
 * falls back to `${oldFile.cacheKey}:${newFile.cacheKey}` for an unkeyed diff, and
 * those loaded-file keys name a path and a revision — nothing about the hunks. Two
 * different shapes of one file at one revision pair (a collapsed view and a full
 * one, a guide card's patch and the diff stream's) then share a single AST entry,
 * and the renderer walks one shape's rows against the other's rendered lines:
 * "deletionLine and additionLine are null". Keying the parse makes the hydrated key
 * `${fingerprint}:hydrated` instead, which is per-shape, and it is what gives the
 * `:compact-partial` and `:collapsed-…` suffixes below something to attach to.
 */
export function patchCacheKey(patchText: string): string {
  const primary = fnv1a(patchText, FNV_OFFSET_BASIS, FNV_PRIME).toString(36)
  const secondary = fnv1a(patchText, SECONDARY_SEED, SECONDARY_MULTIPLIER).toString(36)
  return `solus-diff:${patchText.length}:${primary}:${secondary}`
}
...
// instance itself. @pierre hydrates a partial diff by mutating the metadata it
// was given, so the cached parse must not be the object a renderer holds. A
// shallow copy keeps the expensive part — the parsed hunk and line arrays —
// shared, while `patchCacheKey` is what tells @pierre whether two copies render
// the same rows.
...
export function parsePatchMetadata(patchStr: string): FileDiffMetadata | null {
  ...
  if (parsed === undefined) {
    try {
      const file = parsePatchFiles(patchStr, patchCacheKey(patchStr))[0]?.files[0]
