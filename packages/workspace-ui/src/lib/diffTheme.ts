...
export const DIFFS_THEME_CSS = `
  ...
  /* A compact pane: scale the file-header chrome down a notch so it stays
     proportional. Keyed on the pane, not the window — a diff opened as a
     companion is narrow on any display, and used to draw the roomy header in a
     380px column. The height here is mirrored by DiffStream's
     \`diffHeaderHeight\`, which reads the same pane width through
     \`isCompactPane\`; keep the two in sync (DIFF_HEADER_HEIGHT_COMPACT). */
  ...
  /* A stacked pane: tightest tier so the filename + stats still fit. Height
     mirrors DIFF_HEADER_HEIGHT_PHONE in DiffStream, via \`isStackedPane\`. Must
     follow the compact block so it wins by source order. */
