...
class Dictation {
  ...
  focusLost(el: DictationTarget): void {
    ...
    this.#clearFocusedTarget(el)
  }

  /**
   * Drops focus tracking for `el` one microtask later. A field often loses focus
   * because Svelte removed it from the DOM, and the browser fires `focusout`
   * synchronously inside that block effect, where writing `$state` throws
   * `state_unsafe_mutation`. Deferring keeps a focus handoff correct too: the
   * next field's `focusGained` runs first, so the identity check no longer
   * matches and its tracking survives.
   */
  #clearFocusedTarget(el: DictationTarget): void {
    queueMicrotask(() => {
