<script lang="ts">
  ...
  import BrowserWebviewLayer from "./components/browser/BrowserWebviewLayer.svelte";
  import {
    uploadFileObjects,
  ...
  let directoryPickerIntent = $state<"dispatch" | "open-project">(
    "open-project",
  );
  ...
  $effect(() => {
    if (
      ...
      hasMountedOpenProject &&
      hasMountedDirectoryPicker
    )
      return;
    const warm = () => {
  ...
  const directoryPickerHostLabel = $derived(
    directoryPickerServerId === serversStore.activeServerId
      ? undefined
      : serversStore.servers.find((s) => s.id === directoryPickerServerId)
          ?.label,
  );
  ...
  const paletteCommands = $derived.by(() => {
    ...
    if (canOpenHostLogs) {
      commands.push({
        ...
        group: "General",
        icon: ScrollTextIcon,
        keywords: ["log", "logs", "debug", "diagnostics", "dev.log", "solus.log"],
...
<svelte:boundary onerror={(error) => console.error("renderer crashed", error)}>
  ...
  >
    ...
    {#if importDocOpen}
      {#await import("./components/work/ImportDocDialog.svelte") then importDocModule}
        {@const ImportDocDialog = importDocModule.default}
        <ImportDocDialog
          open={importDocOpen}
          onClose={() => (importDocOpen = false)}
        />
      {/await}
    {/if}
