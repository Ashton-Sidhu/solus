...
- **Surfaces read stores and shell facts, never the tab strip.** The board surfaces (`tasks/`, `work/`, `workspace/`, `document-shell/`, `diagram/`, `session/record/`, `insights/`) are mounted by desktop, web, and phone. To reach a session they use a context command — `revealSession`, `sessionForAgentSession`, `activeSession` — not `tabs`, `tabOrder`, `activeTabId`, `selectTab`, or `tabIdForAgentSession`. The record surfaces (the task, pull request, and works boards; a task, a work, a session record; and the transcript and markdown they render) read `getSurfaceContext()` and ask its `workspace` before a workspace-only command, so a record renders when the only connection is the organization's workspace service (`docs/plans/cloud-console-native-pages.md` §4 and §9; `cloud-service-model.md` §15 "One door"). `bun run lint:surfaces` enforces both. Leaves (rows, chips, cards, renderers) take props alone.
...
A class can be defined correctly and still never reach the DOM — no build error, no warning, nothing in the markup. Read the compiled class list before re-tuning a value. `bun run test:unit` guards these; `lib/utils.test.ts` explains each.
...
Two axes exist. One is a fact, one is a lie:
...
There is no **monitor** axis. Solus renders one density on every display, and users change it with zoom (`docs/plans/single-density.md`). Do not key type or geometry on `screen.width`.
