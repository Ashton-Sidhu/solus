...
export interface SolusAPI {
  ...
  /** The durable prompt queue of a cloud session (cloud-service-model.md §4): a prompt sent while the runner is away waits for it. */
  sessionPromptEnqueue(request: SessionPromptEnqueueRequest): Promise<CloudQueuedPrompt>
  sessionPromptQueueList(sessionId: string): Promise<CloudQueuedPrompt[]>
  /** Withdraws a prompt still waiting; its author or a host administrator. */
  sessionPromptQueueCancel(request: { queueId: string }): Promise<{ cancelled: boolean }>
