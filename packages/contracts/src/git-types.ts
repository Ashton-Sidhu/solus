...
export type GitPullRequestStep =
  | {
    ...
    title: string
    /**
     * The pull request itself, read back from the provider.
     *
     * `url`/`number`/`title` are what the create step itself produced — enough
     * to report and to link a task. This is the provider's own answer, so a
     * client can index a complete pull request without a second round trip and
     * without inventing one. Null when it could not be read: a `gh` CLI
     * fallback that returned no parseable number, or a checkout whose
     * credential does not reach the API. Ask by number in that case.
     */
