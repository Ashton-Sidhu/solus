 ...
 *  build that stored it asked for `drive.file` alone. */
const LEGACY_GRANTED_SCOPES = [GOOGLE_DRIVE_FILE_SCOPE]
...
interface StoredToken {
  ...
  accessToken: string
  expiresAt: number
  /** What Google actually granted, which can be narrower than it was asked for
   *  and narrower than a later build requires. Absent on a grant stored before
   *  this field existed. */
  scopes?: string[]
}
...
/**
 * What the stored grant actually holds, or null when nothing is stored. A grant
...
export async function completeGoogleOAuthCallback(params: URLSearchParams): Promise<GoogleOAuthCallbackResult> {
  ...
  try {
    ...
    // Google states the granted scopes in the exchange response; a user can
    // clear a checkbox on the consent screen, so what was asked for is not
    // proof of what was given.
    ...
    log.warn('google_oauth_callback_failed', { error: message })
    // An unusable keyring is the one failure the user can act on, so it is named
    // rather than folded into the generic retry advice.
    if (err instanceof EncryptionUnavailableError) {
      return callbackPage(500, 'Google connection failed', message)
    }
...
export async function getAccessToken(): Promise<string | null> {
  ...
  client.on('tokens', (tokens) => {
    ...
    // A refresh response may omit `scope`; the grant is unchanged either way, so
