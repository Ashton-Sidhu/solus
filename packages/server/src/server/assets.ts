...
function rawAllowedRoots(ctx: IpcContext | undefined): string[] {
  ...
  // This conversation's own upload folder, so a client that did not compose the
  // message can still render its images from the ref instead of the bytes.
  // Kept last: `roots[0]` stays the project base for relative requests.
  const bucketId = uploadBucketId(ctx)
  if (bucketId) {
    roots.push(join(dataDir(), 'attachments', uploadFolderName(bucketId)))
  }
