...
export const KIND_REGISTRY = {
  ...
  [SPAN_KINDS.toolCall]: {
    description: 'one end-to-end tool invocation, including input generation, permission, and execution',
    fields: [
      TOOL_NAME_FIELD,
  ...
  [SPAN_KINDS.toolInput]: {
    description: 'tool input generation and streaming before execution can start',
    fields: [TOOL_NAME_FIELD],
  },
  [SPAN_KINDS.toolExecution]: {
    description: 'tool execution after its complete input is available',
    fields: [TOOL_NAME_FIELD],
  },
  ...
  [SPAN_KINDS.browserLoad]: {
    description: 'one browser page load, from navigation to the guest reporting it ready',
    fields: [
      ...withGroup('dimension', [
        attr('url', 'url', 'string', 'Address the browser page loaded'),
        attr('viewport', 'viewport', 'string', 'The emulated viewport, e.g. "iPhone 15 — 393×852"'),
        attr('preset', 'preset', 'string', 'Device preset id when the viewport is a named device'),
        attr('branch', 'branch', 'string', 'Branch of the worktree serving the page, when the scan knew one'),
        attr('worktree', 'worktree', 'string', 'Worktree path serving the page, when the scan knew one'),
        attr('host_kind', 'hostKind', 'string', "Which backend rendered it: 'webview' or 'headless'"),
      ]),
      ...withGroup('detail', [
        attr('problem', 'problem', 'string', 'Why the load failed, when it did'),
      ]),
    ],
  },
  [SPAN_KINDS.browserCapture]: {
    description: 'one browser capture: a snapshot or a piece of evidence, with the page\'s Web Vitals',
    fields: [
      ...withGroup('measure', [
        attr('ttfb_ms', 'ttfbMs', 'duration', 'Time to first byte of the browser page'),
        attr('fcp_ms', 'fcpMs', 'duration', 'First Contentful Paint — first text or image painted'),
        attr('lcp_ms', 'lcpMs', 'duration', 'Largest Contentful Paint — when the main content became visible'),
        attr('cls', 'cls', 'number', 'Cumulative Layout Shift — how much content moved after appearing (unitless)'),
        attr('dom_content_loaded_ms', 'domContentLoadedMs', 'duration', 'DOMContentLoaded of the browser page'),
        attr('load_ms', 'loadMs', 'duration', 'Load event of the browser page'),
        attr('element_count', 'elementCount', 'number', 'Interactive elements the snapshot described'),
        attr('console_error_count', 'consoleErrorCount', 'number', 'Console errors the page had accumulated at capture time'),
      ]),
      ...withGroup('dimension', [
        attr('url', 'url', 'string', 'Address that was captured'),
        attr('viewport', 'viewport', 'string', 'The emulated viewport the capture was taken at'),
        attr('preset', 'preset', 'string', 'Device preset id when the viewport is a named device'),
        attr('branch', 'branch', 'string', 'Branch of the worktree serving the page, when the scan knew one'),
        attr('worktree', 'worktree', 'string', 'Worktree path serving the page, when the scan knew one'),
        attr('host_kind', 'hostKind', 'string', "Which backend rendered it: 'webview' or 'headless'"),
        attr('attached_to', 'attachedTo', 'string', 'Where the capture was filed, when it was filed anywhere'),
      ]),
    ],
  },
...
} satisfies Record<SpanKind, KindRegistration>
...
const EVENT_FIELDS = (): RegisteredField[] => [
  ...withGroup('dimension', [
    ...
    column('name', 'name', 'string', 'Kind-specific name: the tool, the operation, the wait'),
    {
      name: 'tool',
      type: 'string',
      description: 'Tool name when the event belongs to a tool call, one of its phases, or its permission wait, e.g. Bash, Read',
      group: 'dimension',
      storage: { source: 'expression', sql: `CASE WHEN kind IN ('${SPAN_KINDS.toolCall}', '${SPAN_KINDS.toolInput}', '${SPAN_KINDS.toolExecution}', '${SPAN_KINDS.permissionWait}') THEN name END` },
    },
  ]),
...
/** Declared column types, by view name then column name. Built once: a result
 *  is typed on every query, and the registry cannot change while the process
 *  runs. `spans` is included so a raw fact-table query still types the columns
 *  it selected, even though it declares no grain. */
const COLUMN_TYPES: ReadonlyMap<string, ReadonlyMap<string, MetricsFieldType>> = new Map(
  [
    ...viewRegistrations().map((registration): [string, RegisteredField[]] =>
      [registration.view, registration.fields]),
    // The fact table is not a registered view, but a query reading it directly
    // still selects columns the registry declares.
    ...[{ view: 'spans', fields: BASE_TABLE_FIELDS }].map(
      (entry): [string, RegisteredField[]] => [entry.view, entry.fields]),
  ].map(([view, fields]) => [
    view,
...
/** The type the registry declares for a column of `view`, or undefined where
 *  the view is unknown or the column is one the query invented. */
export function declaredColumnType(view: string | undefined, column: string): MetricsFieldType | undefined {
  return view === undefined ? undefined : COLUMN_TYPES.get(view)?.get(column)
}
