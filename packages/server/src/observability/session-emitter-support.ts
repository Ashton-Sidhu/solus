import type { NormalizedEvent, UsageData } from '@solus/contracts/types'
import type { SpanAttributes, SpanStatus } from './registries'

export type TurnOutcome = 'completed' | 'interrupted' | 'failed'

/** A value cut to a length cap, and whether cutting it lost anything. */
export interface CappedValue {
  value: string
  truncated: boolean
}

export function capped(value: string, limit: number): CappedValue {
...
export function cappedAttrs(attrs: SpanAttributes | undefined): SpanAttributes {
  ...
  for (const [key, value] of Object.entries(attrs)) {
    result[key] = Object.prototype.toString.call(value) === '[object String]'
      ? capped(String(value), DISPATCH_ATTR_LIMIT).value
      : value
  }
  return result
}
...
export function spanStatusForToolOutcome(
  ...
  if (outcome.exitCode !== undefined && outcome.exitCode !== null && outcome.exitCode !== 0) return 'error'
  return 'ok'
}
