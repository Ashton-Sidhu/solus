...
} satisfies Record<MetricsTimeBucket, string>

const PERCENTILES = new Map<MetricsAggregate['fn'], number>([['p50', 0.5], ['p95', 0.95]])
const PLAIN_AGGREGATES = new Map<MetricsAggregate['fn'], string>([
...
function pinnedKind(filters: MetricsFilter[]): SpanKind | undefined {
  const filter = filters.find((f) => f.field === 'kind' && f.op === 'eq')
  const value = filter === undefined ? '' : String(filter.value)
  if (!registeredKinds.has(value)) return undefined
  // SAFETY: `registeredKinds` holds exactly the members of `SPAN_KINDS`.
  return value as SpanKind
}
...
/** SQLite has no boolean storage class, so a boolean filter binds as 0 or 1. */
...
/** A compiled `WHERE` clause and the values its placeholders bind. */
interface WhereClause {
  clause: string
  params: Array<string | number>
}

function buildWhere(spec: MetricsQuerySpec, kind: SpanKind | undefined): WhereClause {
...
function aggregateItems(aggregates: MetricsAggregate[], kind: SpanKind | undefined): AggregateItem[] {
  return aggregates.map((aggregate) => {
    if (PLAIN_AGGREGATES.get(aggregate.fn) === undefined && PERCENTILES.get(aggregate.fn) === undefined) {
...
function compilePlainAggregation(
  ...
  const selects = [
    ...
    ...aggregates.map((aggregate) => aggregate.expression === null
      ...
      : `${PLAIN_AGGREGATES.get(aggregate.fn)}(${aggregate.expression}) AS ${aggregate.alias}`),
  ]
...
function compileWithPercentiles(
  ...
  const plain = aggregates
    ...
    .filter(({ aggregate }) => PLAIN_AGGREGATES.get(aggregate.fn) !== undefined)
  const percentiles = aggregates
    .map((aggregate, index) => ({ aggregate, index }))
    .filter(({ aggregate }) => PERCENTILES.get(aggregate.fn) !== undefined)

  const plainSelects = [
    ...groupAliases,
    ...plain.map(({ aggregate, index }) => aggregate.expression === null
      ? `COUNT(*) AS ${aggregate.alias}`
      : `${PLAIN_AGGREGATES.get(aggregate.fn)}(${valueAlias(index)}) AS ${aggregate.alias}`),
  ]
  ...
  const percentileCtes = percentiles.map(({ aggregate, index }, order) => {
    ...
    return `__pct${order} AS (\n  SELECT ${keep} FROM (\n    SELECT ${[...groupAliases, value].join(', ')},\n`
      ...
      + `  WHERE __rn = MAX(1, CAST(__cnt * ${PERCENTILES.get(aggregate.fn)} + 0.5 AS INTEGER))\n)`
  })
...
export function compileQuerySpec(spec: MetricsQuerySpec): CompiledQuery {
  ...
  const where = buildWhere(spec, kind)
  const hasPercentile = aggregates.some((aggregate) => PERCENTILES.get(aggregate.fn) !== undefined)
