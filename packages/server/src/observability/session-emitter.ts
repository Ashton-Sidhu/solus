...
/** A tool call's own facts. A call with no parent tool records no parent id
 *  rather than a null one. */
function toolCallAttrs(parentToolUseId: string | undefined, isSubagent: boolean): SpanAttributes {
  const attrs: SpanAttributes = { isSubagent }
  if (parentToolUseId) attrs.parentToolUseId = parentToolUseId
  return attrs
}

/** A background task's own facts; it never blocks the turn it was spawned in. */
function backgroundTaskAttrs(toolUseId: string | undefined): SpanAttributes {
  const attrs: SpanAttributes = { blocking: false }
  if (toolUseId) attrs.toolUseId = toolUseId
  return attrs
}
...
interface TurnState {
  ...
  toolInputStartedAt: Map<string, number>
  toolExecutionStartedAt: Map<string, number>
  toolsWaitingForPermission: Set<string>
  permissionOptions: Map<string, Set<string>>
  permissionToolKeys: Map<string, string>
  rateLimitKey?: string
...
export class SessionEmitter {
  ...
  beginTurn(input: {
    ...
    this.safe(input.sessionId, () => {
      ...
      this.turns.set(input.sessionId, {
        ...
        toolInputStartedAt: new Map(),
        toolExecutionStartedAt: new Map(),
        toolsWaitingForPermission: new Set(),
        permissionOptions: new Map(),
        permissionToolKeys: new Map(),
      })
    })
    return traceId
  }
  ...
  withStep<T>(
    ...
    if (!step) {
      // SAFETY: `run` returns `T | Promise<T>`, and `Promise.resolve` collapses both
      // to `Promise<T>` — a distinction the overload set does not carry through.
      return Promise.resolve(run(() => {})) as Promise<T>
    }
  ...
  onEvent(sessionId: string, event: NormalizedEvent, arrivedAt = Date.now()): void {
    this.safe(sessionId, () => {
      ...
      if (event.type === 'tool_call_complete') {
        ...
        const inputStartedAt = state.toolInputStartedAt.get(key)
        if (inputStartedAt !== undefined) {
          state.toolInputStartedAt.delete(key)
          this.recordToolPhase(state, key, SPAN_KINDS.toolInput, inputStartedAt, arrivedAt, 'ok')
          if (!state.toolsWaitingForPermission.has(key)) state.toolExecutionStartedAt.set(key, arrivedAt)
        }
        const outcome = event.outcome
        if (outcome) {
          const attrs: SpanAttributes = {}
          if (outcome.status) attrs.outcomeStatus = outcome.status
          if (outcome.exitCode !== undefined && outcome.exitCode !== null) attrs.exitCode = outcome.exitCode
        ...
        if (event.completedAtMs !== undefined || outcome !== undefined) {
          this.finishToolExecution(state, key, event.completedAtMs ?? arrivedAt, spanStatusForToolOutcome(outcome))
      ...
      if (event.type === 'tool_result') {
        ...
        this.finishToolExecution(state, key, arrivedAt, event.isError ? 'error' : 'ok')
        state.openSpans.delete(key)
      ...
      if (event.type === 'permission_resolved') {
        ...
        const open = state.openSpans.get(key)
        if (open) {
          const toolKey = state.permissionToolKeys.get(key)
          if (toolKey) {
            state.toolsWaitingForPermission.delete(toolKey)
            state.toolExecutionStartedAt.set(toolKey, arrivedAt)
          }
          state.openSpans.delete(key)
          state.permissionOptions.delete(key)
          state.permissionToolKeys.delete(key)
  ...
  resolvePermission(sessionId: string, questionId: string, optionId: string, endedAt = Date.now()): void {
    this.safe(sessionId, () => {
      ...
      const toolKey = state.permissionToolKeys.get(key)
      if (toolKey) state.toolsWaitingForPermission.delete(toolKey)
      if (granted && toolKey) state.toolExecutionStartedAt.set(toolKey, endedAt)
      if (!granted) {
  ...
  private finish(state: TurnState, status: SpanStatus, endedAt: number): void {
    ...
    // Tool phases end before their parent tool. A provider failure can stop a
    // call while it is still generating input or executing it.
    for (const [key, startedAt] of state.toolInputStartedAt) {
      state.toolInputStartedAt.delete(key)
      this.recordToolPhase(state, key, SPAN_KINDS.toolInput, startedAt, endedAt, status)
    }
    for (const [key, startedAt] of state.toolExecutionStartedAt) {
      state.toolExecutionStartedAt.delete(key)
      this.recordToolPhase(state, key, SPAN_KINDS.toolExecution, startedAt, endedAt, status)
    }
  ...
  private startTool(state: TurnState, event: Extract<NormalizedEvent, { type: 'tool_call' }>, startedAt: number): void {
    ...
    const open = this.newChild(
      ...
      event.toolName,
      startedAt,
      toolCallAttrs(event.parentToolUseId, event.isSubagent === true),
      parent && trace.setSpan(state.rootContext, parent.span),
    )
    if (event.toolInput) this.setToolInput(open, event.toolInput)
    state.toolKeyByIndex.set(event.index, key)
    state.toolSpans.set(key, open)
    state.toolNames.set(key, event.toolName)
    state.openSpans.set(key, open)
    if (state.dimensions?.provider === 'claude-code' && event.toolInput === undefined) {
      state.toolInputStartedAt.set(key, startedAt)
    } else {
      state.toolExecutionStartedAt.set(key, startedAt)
    }
  }
  ...
  private startPermission(
    ...
    const key = this.permissionKey(event.questionId)
    if (state.openSpans.has(key)) return
    const toolKey = event.toolUseId ? this.toolKey(event.toolUseId) : undefined
    const parent = toolKey ? state.toolSpans.get(toolKey) : undefined
    if (toolKey) {
      state.toolExecutionStartedAt.delete(toolKey)
      state.toolsWaitingForPermission.add(toolKey)
      state.permissionToolKeys.set(key, toolKey)
    }
    state.openSpans.set(key, this.newChild(
      state,
  ...
  private startBackgroundTask(
    ...
    state.openSpans.set(key, this.newChild(
      ...
      event.taskId,
      startedAt,
      backgroundTaskAttrs(event.toolUseId),
      parent && trace.setSpan(state.rootContext, parent.span),
    ))
  }
  ...
  private finishToolExecution(
    ...
    const startedAt = state.toolExecutionStartedAt.get(toolKey)
    if (startedAt === undefined) return
    state.toolExecutionStartedAt.delete(toolKey)
    this.recordToolPhase(state, toolKey, SPAN_KINDS.toolExecution, startedAt, endedAt, status)
  }

  private recordToolPhase(
    state: TurnState,
    toolKey: string,
    kind: typeof SPAN_KINDS.toolInput | typeof SPAN_KINDS.toolExecution,
    startedAt: number,
    endedAt: number,
    status: SpanStatus,
  ): void {
    const parent = state.toolSpans.get(toolKey)
    const toolName = state.toolNames.get(toolKey)
    if (!parent || !toolName) return
    const phase = this.newChild(
      state,
      kind,
      toolName,
      startedAt,
      {},
      trace.setSpan(state.rootContext, parent.span),
    )
    this.endChild(state, phase, endedAt, status)
  }
