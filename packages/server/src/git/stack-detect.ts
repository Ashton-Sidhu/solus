...
import type { StackEdge, StackGraph } from '@solus/contracts/stack-types'
...
export interface StackPullRequestInput {
  ...
  baseRef: string
  headRef: string
  /** The head branch lives in another repository, so no local branch can parent it. */
  isFork: boolean
}
...
async function detectStackGraphUncached(deps: StackDetectionDeps): Promise<StackGraph> {
  ...
  for (const pullRequest of summaries) {
    const fetchedHead = fetched
      ? await runAsync('git', ['rev-parse', `refs/solus/pr/${pullRequest.number}`], deps.repoRoot).catch(() => pullRequest.headSha)
      : pullRequest.headSha
    pullRequests.push({
      ...
      isFork: pullRequest.headRepo.isFork,
    })
  }
