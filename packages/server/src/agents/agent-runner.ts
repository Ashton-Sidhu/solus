...
/** An agent run's own facts. A run with no reasoning effort records none rather
 *  than an empty one. */
function agentRunAttrs(promptChars: number, reasoningEffort: string | undefined): SpanAttributes {
  const attrs: SpanAttributes = { promptChars }
  if (reasoningEffort) attrs.reasoningEffort = reasoningEffort
  return attrs
}
...
export class AgentRunner {
  ...
  run(request: AgentRunRequest, sessionState?: AgentRunSessionState): AgentRun {
    ...
    if (request.persistence === 'ephemeral') {
      try {
        agentSpan = startSolusSpan({
          ...
          attrs: agentRunAttrs(request.prompt.length, request.reasoningEffort),
          parent: ROOT_CONTEXT,
        })
      } catch (error) {
    ...
    const done = completion.then<AgentRunResult>(() => ({
      ...
      (result) => {
        const interrupted = result.signal === 'SIGINT' || handle.abortController.signal.aborted
        const failed = result.exitCode !== null && result.exitCode !== 0
        const attrs: SpanAttributes = {
          toolCallCount: result.toolCallCount,
          permissionDenialCount: result.permissionDenials.length,
        }
        if (result.exitCode !== null) attrs.exitCode = result.exitCode
        finishAgentSpan(interrupted ? 'interrupted' : failed ? 'error' : 'ok', attrs)
        return result
      },
      (error) => {
