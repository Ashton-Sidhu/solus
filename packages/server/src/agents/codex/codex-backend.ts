...
export class CodexBackend extends BaseAgentBackend<CodexRunHandle> implements AgentBackend {
  ...
  private mergeSessionListCache(changed: SessionMeta[]): void {
    for (const cacheKey of this.sessionListCache.keys({ includeStale: true })) {
      ...
      const relevant = changed.filter((session) =>
        codexThreadBelongsToProject({ cwd: resolveHomePath(session.cwd) }, cacheKey),
      )
  ...
  private async threadToSessionMeta(
    ...
    const cwd = resolveHomePath(thread.cwd || projectPath || process.cwd())
    ...
    if (msg.method === 'turn/completed') {
      ...
      if (handle?.persistent) {
        this.sessionListCache.invalidateWhere((cacheKey) =>
          codexThreadBelongsToProject({ cwd: resolveHomePath(handle.cwd) }, cacheKey),
        )
  ...
  private onServerRequest(msg: JsonRpcRequest): void {
    ...
    if (msg.method === 'item/tool/call') {
      ...
      const toolStartedAt = z.number().finite().safeParse(params?.startedAtMs)
      if (toolStartedAt.success) toolRequest.startedAtMs = toolStartedAt.data
      this.emit('normalized', sessionId, toolRequest)
      return
    }
    ...
    const startedAt = z.number().finite().safeParse(params?.startedAtMs)
    if (startedAt.success) request.startedAtMs = startedAt.data
    this.emit('normalized', sessionId, request)
  }
  ...
  private async listAllThreads(cwd?: string): Promise<CodexThreadSummary[]> {
    ...
    do {
      ...
      if (cwd) Object.assign(request, { cwd: resolveHomePath(cwd) })
