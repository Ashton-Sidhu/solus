...
function normalizeCodexNotification(method: string, params: any, opts?: { planMode?: boolean; assembledAgentMessages?: boolean }): NormalizedEvent[] {
  ...
  switch (method) {
    ...
    case 'model/rerouted': {
      const fromModel = parsedString(params?.fromModel)
      const toModel = parsedString(params?.toModel)
      if (fromModel === undefined || toModel === undefined) return []
      const rerouted: Extract<NormalizedEvent, { type: 'model_rerouted' }> = {
        type: 'model_rerouted',
        fromModel,
        toModel,
      }
      if (params.reason !== undefined) rerouted.reason = String(params.reason)
      return [rerouted]
    }
...
function normalizeItemCompleted(params: any, opts?: { assembledAgentMessages?: boolean }): NormalizedEvent[] {
  ...
  const complete: Extract<NormalizedEvent, { type: 'tool_call_complete' }> = {
    type: 'tool_call_complete',
    index: 0,
    toolId: item.id,
  }
  const completedAtMs = parsedFiniteNumber(params?.completedAtMs)
  if (completedAtMs !== undefined) complete.completedAtMs = completedAtMs
  if (outcome) complete.outcome = outcome
...
/** A web-search tool call as Solus records its input. A search with no action
 *  states none rather than an empty one. */
...
function codexStartedToolInput(item: any): string | undefined {
  ...
  if (item.type === 'webSearch' && parsedString(item.query) !== undefined) {
    const search: WebSearchToolInput = { query: parsedString(item.query) ?? '' }
    if (item.action) search.action = item.action
    return JSON.stringify(search)
  }
  if (item.type === 'fileChange' && Array.isArray(item.changes)) return JSON.stringify({ changes: item.changes })
  if (item.type === 'imageView' && parsedString(item.path) !== undefined) return JSON.stringify({ path: item.path })
  if (item.type === 'sleep' && parsedFiniteNumber(item.durationMs) !== undefined) {
    return JSON.stringify({ durationMs: item.durationMs })
  }
  if (item.type === 'imageGeneration' && parsedString(item.revisedPrompt) !== undefined) {
...
function codexToolOutcome(item: any): Extract<NormalizedEvent, { type: 'tool_call_complete' }>['outcome'] {
  const status = parsedString(item.status)
  const exitCode = parsedFiniteNumber(item.exitCode)
  const error = parsedString(item.error)
    ?? (item.error === null || item.error === undefined ? undefined : JSON.stringify(item.error))
  ...
  const outcome: Extract<NormalizedEvent, { type: 'tool_call_complete' }>['outcome'] = {}
  if (status) outcome.status = status
  else if (item.success === true) outcome.status = 'completed'
  else if (item.success === false) outcome.status = 'failed'
  if (exitCode !== undefined) outcome.exitCode = exitCode
  if (error) outcome.error = error
  if (declined) outcome.declined = true
  if (durationMs !== undefined) outcome.durationMs = durationMs
  return outcome
}

/** The started-at stamp a tool-call event carries, where the run reported one. */
function startedAtField(params: any): { startedAtMs?: number } {
  const startedAtMs = parsedFiniteNumber(params?.startedAtMs)
