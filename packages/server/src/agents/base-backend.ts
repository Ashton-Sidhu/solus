...
import type { RunHandle, RunTrackingSnapshot } from './agent-backend'
import type { EnrichedError } from '@solus/contracts/types'
import { createLogger } from '../logger'

const log = createLogger('AgentBackend', 'base-backend.ts')
...
export class BaseAgentBackend<H extends RunHandle = RunHandle> extends EventEmitter {
  ...
  runTrackingSnapshot(): RunTrackingSnapshot {
    return {
      activeRunSessionIds: [...this.activeRuns.keys()],
      pendingRunSessionIds: this.pendingRuns.map((handle) => handle.agentSessionId),
      finishedRunSessionIds: [...this.finishedRuns.keys()],
    }
  }
  ...
  protected promoteToActive(handle: H, sessionId: string): void {
    // A provider that reports a new thread id mid-run re-keys `activeRuns` here.
    // The ControlPlane addresses the run by the id it last saw, so a re-key it
    // does not also see makes `isSessionRunning` answer false for a live run and
    // the watchdog kills it. Log the transition so that is visible in dev.log.
    if (handle.agentSessionId && handle.agentSessionId !== sessionId) {
