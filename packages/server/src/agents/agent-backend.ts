...
/** What the backend currently believes about its own runs. Diagnostic only:
 *  read when the run watchdog is about to declare a session dead, so the log
 *  says whether the run vanished, finished without an `exit`, or is alive
 *  under a different provider thread id. */
export interface RunTrackingSnapshot {
  activeRunSessionIds: string[]
  pendingRunSessionIds: Array<string | null>
  finishedRunSessionIds: string[]
}
...
export interface AgentBackend extends EventEmitter {
  ...
  /** Diagnostic only; every real backend inherits it from BaseAgentBackend. */
