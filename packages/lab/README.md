...
- `cloud-sessions` (personal flavor only; cloud-service-model.md §18–§19, the P2 exit test) — a runner streams a slow turn whose rows reach the service as they land; the runner is killed mid-turn; the transcript so far is readable and the session listed with the runner dead; alice's prompt waits on the cloud queue; the runner restarts on its data directory, claims the prompt, runs it on the host login, and the new rows reach the service; a waiting prompt can be withdrawn, a dispatched one cannot.
