...
{#if open}
  ...
  >
    <div
      class="w-full max-w-[25rem] rounded-2xl border border-[var(--solus-popover-border)] bg-[color-mix(in_srgb,var(--solus-container-bg)_94%,transparent)] p-6 text-center shadow-[var(--solus-popover-shadow),0_1.5rem_5rem_rgba(0,0,0,0.16)] backdrop-blur-2xl dark:shadow-[var(--solus-popover-shadow),0_1.5rem_5rem_rgba(0,0,0,0.45)]"
      role="dialog"
      aria-modal="true"
    ...
    >
      ...
      <div class="mt-6 grid gap-2">
        <button
          class="w-full cursor-pointer rounded-lg border-0 bg-[var(--solus-accent)] px-4 py-2.5 text-sm font-semibold text-white transition-[filter,transform] hover:brightness-105 active:scale-[0.985] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--solus-accent)]"
          onclick={download}
        >
          Download Solus
        </button>
        <button
          class="w-full cursor-pointer rounded-lg border-0 bg-transparent px-4 py-2 text-sm font-medium text-[var(--solus-text-tertiary)] transition-colors hover:bg-[var(--solus-surface-hover)] hover:text-[var(--solus-text-primary)] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--solus-accent)]"
          onclick={close}
        >
          Keep exploring
        </button>
