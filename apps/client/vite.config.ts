...
export default defineConfig({
  ...
  server: {
    ...
    port: 5174,
    fs: {
      // Allow serving from the repo root so renderer/* works, plus whichever
      // checkout's node_modules Node resolved (see devServeRoots).
      allow: devServeRoots(resolve(__dirname, '../..')),
    },
  },
  build: {
    ...
    // Skip the gzip pass Vite runs over every chunk just to print a size
    // column. Measure transfer size with a dedicated tool when it matters.
