...
export function sqlWindow(sql: string, now: number): SqlWindow {
...
/** These rollups alias their own aggregates — `count(*) as events` — so no
 *  registry field declares them and the server would send them untyped too. */
function aliasedColumns(names: string[]): MetricsResultColumn[] {
...
function ranked(
  ...
  return {
    columns: aliasedColumns(columns),
...
const WHERE_TIME_GOES: MetricsQueryResult = {
  columns: aliasedColumns(['kind', 'events', 'total_seconds']),
  rows: [
...
const AGENT_SPEND_BY_SERVICE: MetricsQueryResult = {
  columns: aliasedColumns(['service', 'runs', 'avg_seconds', 'total_seconds']),
  rows: [
...
const PERMISSION_WAITS: MetricsQueryResult = {
  columns: aliasedColumns(['model', 'waits', 'total_seconds']),
  rows: [
...
export const DEMO_ANSWERS: DemoAnswer[] = [
  ...
  },
  {
    ...
    sql: '',
    result: (turns) => ({
      columns: aliasedColumns(['project_root', 'turns', 'spend_usd']),
