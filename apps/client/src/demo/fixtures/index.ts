...
/** One captured fixture file, as Vite hands its parsed JSON default export back. */
interface FixtureModule<T> {
  default: T
}

// SAFETY: `eager: true` resolves every match at build time, so each entry is the
// module record Vite generates for a parsed JSON file.
const modules = import.meta.glob('./data/*.json', { eager: true }) as Record<string, FixtureModule<unknown>>

function fixture<T>(name: string, fallback: T, isEmpty: (value: T) => boolean): T {
  // SAFETY: each caller names the `DemoFixtures` field the captured file holds, and a
  // capture that does not match is replaced by `fallback` below.
...
const emptyObject = <T>(value: T): boolean => Object.keys(value ?? {}).length === 0
