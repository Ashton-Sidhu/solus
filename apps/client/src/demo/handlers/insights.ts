...
export function registerInsightsHandlers(backend: DemoServer): void {
  ...
  backend.register('metricsDistinctValues', (args) => DEMO_DISTINCT_VALUES.get(arg<string>(args, 0)) ?? [])
  backend.register('metricsListSavedQueries', () => savedQueries)
  backend.register('metricsSaveQuery', (args) => {
    const query = arg<SavedMetricsQuery>(args, 0)
  ...
  backend.register('metricsQuery', (args): MetricsQueryResult => {
    const spec = arg<MetricsQuerySpec>(args, 0)
    return turnListingResult(within(spec.timeRange ?? {}))
  })

  // The listing the page opens on. It is not SQL the visitor can see, so it is
  // answered from the same recorded turns rather than routed through the
  ...
  backend.register('metricsSessionSummary', (args) => demoSessionSummary(arg<string>(args, 0), turns))
}
