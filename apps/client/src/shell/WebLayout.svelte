...
import {
  ...
  import MobileComposerActions from "./mobile/MobileComposerActions.svelte";
  import WebMobileLayout from "./mobile/WebMobileLayout.svelte";
  ...
  // Absent is the branch review — the whole branch against its target, which
  // the review surface resolves live — the same reading the desktop rail's
  // "Review changes" opens.
  let diffScope = $state<DiffScope | undefined>({ kind: "session" });
  ...
  /** The branch's own changes, on the diff — what the phone's task sheet opens
   *  where the desktop rail offers "Review changes". */
  function openBranchReview() {
    const sourceTabId = session.activeTabId;
...
{#if hasMountedMobile}
  <div class="w-full h-full" class:mode-hidden={!isMobile}>
    <WebMobileLayout
      ...
      onReviewBranch={openBranchReview}
    />
  </div>
{/if}
