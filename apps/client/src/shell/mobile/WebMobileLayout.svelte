<script lang="ts">
  ...
  import { MOBILE_STATE_INK } from "./lib/mobile-task-row";
  import {
    sessionTitle,
    getStatusIcon,
    getStatusLabel,
  ...
  interface Props {
    ...
    onToggleDiff: () => void;
    /** Open the branch's own changes on the diff — the phone's "Review changes". */
    onReviewBranch: () => void;
  }
  let {
    chatContent,
  ...
  // The branch, and the pull request that stands for it, on the same mono line
  // as the path: a phone has no project rail to keep them in. Both read the
  // started session's live environment — a draft still says where it will
  // start on the destination strip under the composer.
  const environment = $derived(
    sessionStarted ? environmentStore.environmentFor(sess?.run) : null,
  );
  const branchLabel = $derived(environment ? mobileBranchLabel(environment) : null);
  const currentBranch = $derived(environment ? mobileCurrentBranch(environment) : null);
  const environmentServerId = $derived(
    sessionStarted ? session.serverIdFor(session.activeTabId) : null,
  );
  const environmentCtx = $derived(
    environment && environment.cwd !== "~"
      ? session.ctxForEnvironment(environment.cwd, environment.checkout, session.activeTabId)
      : null,
  );
  const branchPullRequest = $derived(
    pullRequests.projects
      ...
      ?.prForBranch(currentBranch) ?? null,
  );
  const openPullRequest = $derived.by(() => {
    const row = mobilePullRequestRow(environment?.status, branchPullRequest);
    return row.kind === "open" ? row : null;
  });

  // The rail watches details for the conversation on screen; here the navbar
  // is that surface, so it holds the same watch. Details are what carry the
  // uncommitted counts and the branch's pull request URL.
  $effect(() => {
    if (!runtime.isMobileViewport || !environment || !environmentServerId) return;
    if (!environment.cwd || environment.cwd === "~") return;
    return environmentStore.watchDetails(environmentServerId, environment.cwd);
  });

  // Track the visible environment; the project store owns request sharing and
  // retries. Its index writes must not become dependencies of this effect.
  $effect(() => {
    const url = environment?.status?.prUrl;
    const ctx = environmentCtx;
    if (!runtime.isMobileViewport || !url || !currentBranch || !ctx || !environmentServerId) return;
  ...
  >
    <div class="mh-navbar-top">
      ...
      >
        ...
        <!-- Path, branch, pull request and state on one mono line: where this
             session lives, what it is doing right now. The path and the branch
             each give way on their own; the marks after them never do, so a
             long branch name cannot push the running state off the line. -->
        <div class="mh-navbar-path">
          {#if headerPath}<span class="mh-navbar-path-segment">{headerPath}</span>{/if}
          {#if branchLabel}
            {#if headerPath}<span class="mh-navbar-path-dot">·</span>{/if}
            <span class="mh-navbar-branch" title={branchLabel}>
              {#if environment?.isolated}<GitForkIcon size={12} />{:else}<GitBranchIcon size={12} />{/if}
              <span class="mh-navbar-path-segment">{branchLabel}</span>
            </span>
          {/if}
          {#if openPullRequest}
            <span class="mh-navbar-path-dot">·</span>
            <!-- An open pull request, in the host's green — a draft stays
                 neutral, because it is not asking to land yet. -->
            <span
              class="mh-navbar-pr"
              style="color:{openPullRequest.draft ? 'var(--muted-foreground)' : MOBILE_STATE_INK.success}"
              role="img"
              aria-label={openPullRequest.draft
                ? `Draft pull request #${openPullRequest.number}`
                : `Open pull request #${openPullRequest.number}`}
            >
              <GitPullRequestIcon size={12} />#{openPullRequest.number}
            </span>
          {/if}
          {#if (headerPath || branchLabel || openPullRequest) && stateIcon}<span class="mh-navbar-path-dot">·</span>{/if}
          {#if stateIcon}
  ...
  >
    {#if !sessionStarted}
      <div class="mobile-destination-strip">
        <InputBarHeader active={runtime.isMobileViewport} />
      </div>
    {/if}
    <div
      class="mobile-pill"
...
<style>
  ...
  /* A flex row rather than one truncating block: the path and the branch are
     the two members allowed to shrink, and each truncates on its own, so the
     marks after them keep their width. */
  .mh-navbar-path {
    display: flex;
    align-items: center;
    justify-content: center;
  ...
  .mh-navbar-branch > :global(svg),
  .mh-navbar-pr > :global(svg) {
    flex-shrink: 0;
  }
