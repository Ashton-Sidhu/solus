<script lang="ts">
  ...
  import { browserOverlays } from "@solus/workspace-ui/components/browser/lib/browser-overlays.svelte";
  import BrowserSkeleton from "@solus/workspace-ui/components/browser/BrowserSkeleton.svelte";
  import { nativeSurfaces } from "@solus/workspace-ui/components/browser/lib/native-surface-coordinator.svelte";
  import { stageDrag } from "@solus/workspace-ui/components/browser/lib/stage-drag.svelte";
  import {
    BROWSER_BLANK_URL,
    browserProfilePartition,
  } from "@solus/contracts/browser-types";
  import { placeGuest } from "@solus/workspace-ui/components/browser/lib/stage-math";
  import {
    browserGuest,
    shouldUseNativeBrowser,
    supportsNativeBrowser,
  } from "@solus/workspace-ui/components/browser/lib/browser-guest";
...
{#if nativePages.length}
    ...
    {#each nativePages as entry (`${entry.page.browserPageId}:${nativeSurfaces.generationOf(
      ...
      <!-- Keep the guest above the maximized pane (1000), below app overlays. -->
      <div
        class="fixed"
        ...
        style:z-index={placement.layer === "maximized" ? 1001 : 10}
      >
        <div
