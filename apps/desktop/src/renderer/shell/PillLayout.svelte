<script lang="ts">
  ...
  import InputToolbar from "@solus/workspace-ui/components/input/InputToolbar.svelte";
  ...
  import PaneChrome from "@solus/workspace-ui/components/ui/PaneChrome.svelte";
  // Eager, unlike the surfaces below: these are what cover an async boundary,
  // so they cannot sit behind one themselves.
  import PlanModalSkeleton from "@solus/workspace-ui/components/plan/PlanModalSkeleton.svelte";
  import DocumentModalSkeleton from "@solus/workspace-ui/components/document-modal/DocumentModalSkeleton.svelte";
  import DiagramShellSkeleton from "@solus/workspace-ui/components/diagram/DiagramShellSkeleton.svelte";
  import SettingsPageSkeleton from "@solus/workspace-ui/components/settings/SettingsPageSkeleton.svelte";
  import TasksPageSkeleton from "@solus/workspace-ui/components/tasks/TasksPageSkeleton.svelte";
  import AutomationsPageSkeleton from "@solus/workspace-ui/components/automations/AutomationsPageSkeleton.svelte";
  import ListPageSkeleton from "@solus/workspace-ui/components/ui/list-page/ListPageSkeleton.svelte";
  import LazyRoute from "@solus/workspace-ui/components/layout/LazyRoute.svelte";
  // The PR page owns its one loading state. Keep it eager so Pill mode does
  // not add a separate route-module loading state before it.
  import PrsPage from "@solus/workspace-ui/components/prs/PrsPage.svelte";
  import { requestInputFocus } from "@solus/workspace-ui/lib/inputFocus";
  import { retainedConversationTabIds } from "@solus/workspace-ui/components/layout/lib/workspace-body";
  ...
  const windowCtx = getClientShellContext();
  ...
  const isEditorMode = $derived(!windowCtx.isOverlayWindow);
  ...
  /** ⌘Enter in the pill. The session starts in the background — listed, never
   *  activated — and the dock keeps composing for a fresh draft aimed at the
   *  same place, so the pill can fire off a run of prompts without opening. */
  function startPillDraftInBackground(text: string): boolean {
    const draft = pillDraft;
    if (!draft) return false;
    return session.startDraftInBackground(
      draft.id,
      text,
      router.leadingPane.id,
    );
  }
  ...
  <div class="relative flex w-full flex-col" data-solus-ui>
      ...
      <div class="pill-body-reveal no-drag" class:pill-body-open={bodyOpen}>
        ...
        {#if router.at("settings")}
          <div style="height:var(--pill-body-max);overflow:hidden">
            <LazyRoute load={() => import("@solus/workspace-ui/components/settings/SettingsPage.svelte")}>
              {#snippet fallback()}
        ...
        {:else}
          {#if hasMountedWorkspace}
            <LazyRoute load={() => import("@solus/workspace-ui/components/workspace/WorkspacePage.svelte")}>
              {#snippet fallback()}
                <ListPageSkeleton
          ...
          {#if router.at("automations")}
            <div style="height:var(--pill-body-max);overflow:hidden;display:flex;flex-direction:column">
              <LazyRoute load={() => import("@solus/workspace-ui/components/automations/AutomationsPage.svelte")}>
                {#snippet fallback()}
          ...
          {#if router.at("tasks")}
            <div class="flex flex-col overflow-hidden h-[var(--pill-body-max)]">
              <LazyRoute load={() => import("@solus/workspace-ui/components/tasks/TasksPage.svelte")}>
                {#snippet fallback()}
                  <TasksPageSkeleton />
                {/snippet}
              </LazyRoute>
          ...
          <div class:tab-hidden={!conversationPoolVisible}>
              <div class="relative" style="{showPillDiagram || pillGoalSessionId ? 'height:var(--pill-body-max)' : 'max-height:var(--pill-body-max)'}">
                ...
                {#if showPillDiagram}
                  ...
                  <LazyRoute load={() => import("@solus/workspace-ui/components/diagram/DiagramShell.svelte")}>
                    {#snippet fallback()}<DiagramShellSkeleton />{/snippet}
                    {#snippet children(DiagramShell)}
                    <DiagramShell
              ...
              {#if conversationPoolVisible && pillWorkModal && !isEditorMode && pillWorkModal.type !== "diagram"}
                <LazyRoute load={() => import("@solus/workspace-ui/components/document-modal/DocumentModal.svelte")}>
                  {#snippet fallback()}
                  ...
                  />
                  {/snippet}
                  {#snippet children(DocumentModal)}
                  <DocumentModal
              ...
              {:else if conversationPoolVisible && pillPlanModal && !isEditorMode}
                <LazyRoute load={() => import("@solus/workspace-ui/components/plan/PlanModal.svelte")}>
                  {#snippet fallback()}<PlanModalSkeleton />{/snippet}
                  {#snippet children(PlanModal)}
    ...
    <div class="relative" style="z-index:15;margin-bottom:0.625rem">
      ...
      >
        ...
        <!-- The `composer` container, declared here rather than on the pill card
        ...
        <div class="@container/composer px-1.5 pb-1.5 pt-1" data-composer-surface>
          ...
          <InputBar
            ...
            onDispatchInBackground={pillDraft
              ? startPillDraftInBackground
              : undefined}
            bind:prompt
          >
