...
async function main(): Promise<void> {
  ...
  // browser page in. Playwright's is the only browser available here, and it is
...
function installShutdownHandlers(core: BootCore, closeBrowserHost: (() => Promise<void>) | null): void {
  ...
  const shutdown = async (signal: NodeJS.Signals) => {
    ...
    try {
      try {
        // Before the server, so no verb can open a guest into a browser that is
        // already closing. A Chromium that outlives this process is the worst
        // kind of leak: nothing left is holding its pid.
        if (closeBrowserHost) await closeBrowserHost()
        await core.shutdown()
      } finally {
