	...
	<main class="flex-1 min-w-0">
		...
		<div class="flex flex-col text-base/7 sm:text-[15px] sm:leading-[1.8] max-[1440px]:sm:text-[14px] text-[#6B6158]">
			...
			<section id="overview" class="reveal py-10 border-b border-[rgba(0,0,0,0.06)] first:pt-0">
				...
				<p>
					Solus is a workspace for Claude Code and Codex — one place to plan, review, and
					ship agent work. On macOS it floats above whatever you're doing, one keystroke away; from
			...
			<section id="review" class="reveal py-10 border-b border-[rgba(0,0,0,0.06)]">
				...
				<h3 class="text-[13px] font-semibold tracking-[0.05em] uppercase text-[#A09488] mb-1 mt-8">Review commands</h3>
				<p class="text-base/7 sm:text-[14px]">
					You can also request a review from any conversation. Type a
					<strong class="text-[#1A1714] font-medium">/review</strong> command and the report
					comes back as a card in the conversation; the review itself runs in a hidden session,
					so the transcript stays clean. Works with both Claude Code and Codex.
				</p>
				{@render kbTable([
					['/review', 'Review the working tree — staged, unstaged, and untracked changes'],
					['/review:session', "Review this session's changes"],
					['/review:branch', 'Review the branch against its target branch'],
					['/review:pr', "Review the current branch's pull request"],
					['/review <PR URL>', 'Review the pull request at a GitHub URL — any PR you can access, not just ones in the open project'],
				])}
			...
			<section id="connections" class="reveal py-10 border-b border-[rgba(0,0,0,0.06)]">
				...
				<ul class="mt-3 flex flex-col gap-3 list-none p-0">
					{#each [
						...
						['Connect from the browser', `Open a pairing link on the device (AirDrop, Messages, etc.) and the address and token are read from the URL. Or use <strong class="text-[#1A1714] font-medium">Manual setup</strong> and enter the address and code. A bare address such as <code class="text-[12px] font-mono bg-[rgba(0,0,0,0.04)] px-1.5 py-0.5 rounded">192.168.1.42</code> assumes port 3000; write the port only when the server uses another one.`],
...
solus pair                            # create a one-time client pairing link</div>
			...
			<section id="settings" class="reveal py-10 border-b border-[rgba(0,0,0,0.06)]">
				...
				<p>
					Open Settings with {@render kbd('⌘,')}. Preferences live on the host and stay in sync
					across every window and device connected to it. Device-specific state — zoom,
					keybindings, and pane sizes — stays on each device.
				</p>
			...
			<section id="keybindings" class="reveal py-10">
				...
				<p>
					...
					or directly from the shortcuts overlay ({@render kbd('⌘/')}) — click any row to
					rebind it. Web-client variants are noted where they differ.
				</p>
